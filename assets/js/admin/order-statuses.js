/* global vrpaymentOrderStatusesLocalizeScript, ajaxurl */
( function( $, data, wp, ajaxurl ) {
	$( function() {
		var $tbody = $('.vrpayment-order-statuses-rows'),
			$save_button = $('.vrpayment-order-status-save');
		...
		var $row_template = templatesExist('vrpayment-order-statuses-row') ? wp.template('vrpayment-order-statuses-row') : null,
			$blank_template = templatesExist('vrpayment-order-statuses-row-blank') ? wp.template('vrpayment-order-statuses-row-blank') : null;
		...
		var OrderStatus = Backbone.Model.extend({
			save: function( changes ) {
				$.post( ajaxurl, {
					action : 'vrpayment_custom_order_status_save_changes',
					vrpayment_order_statuses_nonce : data.vrpayment_order_statuses_nonce,
					changes,
		...
		var OrderStatusView = Backbone.View.extend({
			...
			initialize: function() {
				...
				$( document.body ).on( 'click', '.vrpayment-order-status-add-new', { view: this }, this.configureNewOrderStatus );
			...
			initRow: function( rowData ) {
				...
				$tr.find( '.view' ).show();
				$tr.find( '.edit' ).hide();
				$tr.find( '.vrpayment-order-status-edit' ).on( 'click', { view: this }, this.onEditRow );
				$tr.find( '.vrpayment-order-status-delete' ).on( 'click', { view: this }, this.onDeleteRow );
			},
			configureNewOrderStatus: function( event ) {
				...
				$( this ).WCBackboneModal({
					template : 'vrpayment-order-statuses-configure',
					variable : {
						key,
						action: 'create',
					},
			...
			onConfigureOrderStatusSubmitted: function( event, target, posted_data ) {
				if ( target === 'vrpayment-order-statuses-configure' ) {
					const view = event.data.view;
					const model = view.model;
					const isNewRow = posted_data.key.includes( 'vrpayment_custom_order_status_' );
					const rowData = {
						...posted_data,
					};
			...
			onLoadBackboneModal: function( event, target ) {
				if ( 'vrpayment-order-statuses-configure' === target ) {
			...
			onDeleteRow: function( event ) {
				...
				$.post( ajaxurl, {
					action: 'vrpayment_custom_order_status_delete',
					vrpayment_order_statuses_nonce: data.vrpayment_order_statuses_nonce,
					key: key
				}, function( response ) {
					if ( response.success ) {
		...
		OrderStatusView.prototype.showErrorModal = function(message) {
			...
			var modalHtml = `
				<div id="vrpayment-error-modal" class="wc-backbone-modal wc-modal-shipping-zone-method-settings" style="max-width: 400px; margin: auto;">
			...
			$('#vrpayment-error-modal').WCBackboneModal();

			// Remove modal from DOM when closed
			$(document).on('click', '.wc-backbone-modal .modal-close', function() {
				$('#vrpayment-error-modal').remove();
			});
		};
	});
})( jQuery, vrpaymentOrderStatusesLocalizeScript, wp, ajaxurl );
