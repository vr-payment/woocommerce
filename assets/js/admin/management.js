...
jQuery(
	function ($) {
		...
		var wc_vrpayment_management = {

			init : function () {
				this.handle_refund_button();
				this.show_refund_states();
				$( '#woocommerce-order-items' ).off( 'click.woo-vrpayment' );
				$( '#woocommerce-order-items' ).on(
					'click.woo-vrpayment',
					'button.vrpayment-completion-show',
					{
						self : this
					},
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-vrpayment',
					'button.action-vrpayment-completion-cancel',
					{
						self : this
					},
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-vrpayment',
					'button.action-vrpayment-completion-execute',
					{
						self : this
					},
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-vrpayment',
					'button.action-vrpayment-void-cancel',
					{
						self : this
					},
					this.cancel_void
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-vrpayment',
					'button.action-vrpayment-void-execute',
					{
						self : this
					},
					this.execute_void
			...
			handle_refund_button : function () {
				if ($( 'span#vrpayment-remove-refund' ).length > 0) {
			...
			show_refund_states : function () {
				$( 'tbody#order_refunds div.vrpayment-refund-status' )
				.each(
					function () {
						...
						if (state == 'failure') {
							refund_thumb
							.addClass( 'vrpayment-refund-state-failue' );
						} else if (state == 'pending'
						|| state == 'sent'
						|| state == 'created') {
							refund_thumb
							.addClass( 'vrpayment-refund-state-pending' );
						}
					}
				);
			...
			show_completion : function (event) {
				...
				$( 'div.wc-order-add-item' ).children(
					'button.vrpayment-completion-button'
				).each(
					function (key, value) {
						$( value ).show();
			...
			update_taxes_for_line_items : function () {
				var initial_amount = $( this ).data( 'vrpayment-initial-amount' );
			...
			execute_completion : function () {
				...
				if (window.confirm( vrpayment_admin_js_params.i18n_do_completion )) {
					...
					var data = {
						action : 'woocommerce_vrpayment_execute_completion',
			...
			show_void : function () {
				...
				$( 'div.wc-order-add-item' ).children( 'button.vrpayment-void-button' )
				.each(
					function (key, value) {
						$( value ).show();
						$( 'div.refund-actions' ).prepend( $( value ) );
			...
			execute_void : function () {
				...
				if (window.confirm( vrpayment_admin_js_params.i18n_do_void )) {

					var data = {
						action : 'woocommerce_vrpayment_execute_void',
						order_id : woocommerce_admin_meta_boxes.post_id,
			...
			update_order : function () {
				...
				var data = {
					action : 'woocommerce_vrpayment_update_order',
					order_id : woocommerce_admin_meta_boxes.post_id,
					security : woocommerce_admin_meta_boxes.order_item_nonce
				};
