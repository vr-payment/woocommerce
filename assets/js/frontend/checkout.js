...
/* global jQuery document window vrpayment_js_params */

jQuery(
	function ($) {
		var wc_vrpayment_checkout = {
			payment_methods : {},
			integrations: {
				LIGHTBOX: 'lightbox',
				IFRAME: 'iframe',
			 ...
			 */
			info : function () {
				var versions = window.vrpayment_js_params.versions || {};
				var info = [
					{library: 'integration', version: window.vrpayment_js_params.integration},
					...
					{library: 'woo-vrpayment', version: versions.vrpayment || null},
				];
				console.table( info, [] );
			},
			...
			init : function () {
				// Payment methods.
				this.$checkout_form.off( 'click.woo-vrpayment' ).on(
					'click.woo-vrpayment',
				...
				this.$checkout_form.off( 'button#place_order' ).on(
					...
					{ self : this },
					function (event) {
						if (vrpayment_js_params.integration === window.wc_vrpayment_checkout.integrations.LIGHTBOX ) {
							return false;
						}
					}
				);
				...
				if ($( document.body ).hasClass( 'woocommerce-order-pay' )) {
					...
					this.$order_review.off( 'click.woo-vrpayment' ).on(
						'click.woo-vrpayment',
						'input[name="payment_method"]',
						{
							self : this
				...
				window.wc_vrpayment_checkout = this;
			},
			...
			payment_method_click : function (event) {
				...
				if ( vrpayment_js_params.integration === self.integrations.PAYMENT_PAGE ) {
					return;
				}
				...
				var configuration = $( "#vrpayment-method-configuration-" + current_method );
			...
			enable_place_order_button: function () {
				...
				this.$order_button.removeClass( 'vrpayment-disabled' );
			},

			disable_place_order_button: function () {
				this.$order_button.prop( 'disabled', true );
				this.$order_button.addClass( 'vrpayment-disabled' );
			},
			...
			is_supported_method : function (method_id) {
				if (!method_id || method_id === 'vrpayment_zero') {
					return false;
				}
				return method_id && (method_id.indexOf( 'vrpayment_' ) === 0);
			},
			...
			register_method : function (method_id, configuration_id, container_id) {
				...
				if (this.checkout_form_identifier === '#order_review') {
					this.$checkout_form.off( 'submit.vrpayment' )
					.on(
						'submit.vrpayment',
						function () {
					...
					this.$checkout_form.off( 'checkout_place_order_' + method_id + '.vrpayment' )
					.on(
						'checkout_place_order_' + method_id + '.vrpayment',
						function () {
							return self.process_submit( method_id );
						}
					);
			...
			process_submit : function (method_id) {
				...
				if ( ! has_full_address) {
					...
					this.submit_error( vrpayment_js_params.i18n_not_complete );
					return false;
				}
						...
						$.ajax(
							{
								...
								success: function ( result ) {
									...
									if (typeof result.vrpayment == 'undefined') {
			...
			process_order_created : function (data, textStatus, jqXHR) {
				...
				if (vrpayment_js_params.integration && vrpayment_js_params.integration === self.integrations.LIGHTBOX ) {
					...
					if ( ! has_full_address) {
						...
						self.submit_error( vrpayment_js_params.i18n_not_complete );
						return false;
					}
					var selected_payment_method = $( 'input[name=payment_method]:checked' ).val();
					var paymentMethodConfigurationId = $( '#vrpayment-method-configuration-' + selected_payment_method ).data( 'configuration-id' );
					...
					if (typeof data.vrpayment == 'undefined') {
						return false;
					}
