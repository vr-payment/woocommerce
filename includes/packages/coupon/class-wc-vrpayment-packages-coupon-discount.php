...
/**
 * Class WC_VRPayment_Packages_Coupon_Discount.
 *
 * This class handles the required unique ids.
 */
class WC_VRPayment_Packages_Coupon_Discount {

	const VRPAYMENT_COUPON = 'Coupon';
	...
	public static function init() {
		...
		add_filter(
			'wc_vrpayment_packages_coupon_discount_totals_including_tax_from_cart',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_vrpayment_packages_coupon_discount_totals_including_tax_from_line_items',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_vrpayment_packages_coupon_cart_has_coupon_discounts_applied',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_vrpayment_packages_coupon_line_items_have_coupon_discounts',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_vrpayment_packages_coupon_discounts_applied_by_cart',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_vrpayment_packages_coupon_percentage_discounts_by_item',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_vrpayment_packages_coupon_process_line_items_with_coupons',
			array(
				__CLASS__,
	...
	public static function copy_total_coupon_discount_meta_data_to_order_item( WC_Order_Item_Product $item, $cart_item_key, $values, WC_Order $order = null ) { //phpcs:ignore
		$coupon_discount = apply_filters( 'wc_vrpayment_packages_coupon_percentage_discounts_by_item', $cart_item_key ); //phpcs:ignore
		$item->add_meta_data( '_vrpayment_coupon_discount_line_item_discounts', $coupon_discount );
		return $item;
	}
	...
	public static function get_coupons_discount_totals_including_tax_from_cart( $currency ) {
		...
		$order_id = $session->get( 'vrpayment_order_id' );
		$cart = WC()->cart;
		...
		foreach ( $cart->get_coupon_discount_totals() as $coupon_discount_total ) {
			$coupons_discount_total += WC_VRPayment_Helper::instance()->round_amount( $coupon_discount_total, $currency );
		}
	...
	public static function cart_has_coupon_discounts_applied( $currency ) {
		$discount = apply_filters( 'wc_vrpayment_packages_coupon_discount_totals_including_tax_from_cart', $currency ); //phpcs:ignore
		return $discount > 0;
	}
	...
	public static function get_coupons_discount_totals_including_tax_from_line_items( array $line_items, $currency ) {
		...
		foreach ( $line_items as $line_item ) {
			if ( $line_item->getType() == \VRPayment\Sdk\Model\LineItemType::DISCOUNT ) {
				$line_item_amount_including_tax = abs( $line_item->getAmountIncludingTax() );
				$coupons_discount_total += WC_VRPayment_Helper::instance()->round_amount( $line_item_amount_including_tax, $currency );
			}
		}
	...
	public static function line_items_have_coupon_discounts( $line_items, $currency ) {
		$discount = apply_filters( 'wc_vrpayment_packages_coupon_discount_totals_including_tax_from_line_items', $line_items, $currency ); //phpcs:ignore
		return $discount > 0;
	}
	...
	public static function get_coupon_percentage_discount_by_item( string $item_key ) {
		$coupon_discounts_applied = apply_filters( 'wc_vrpayment_packages_coupon_discounts_applied_by_cart', WC()->cart ); //phpcs:ignore
	...
	public static function process_line_items_with_coupons( array $line_items, float $expected_sum, string $currency ) {
		...
		$amount = WC_VRPayment_Helper::instance()->get_total_amount_including_tax( $line_items, $exclude_discounts );
		$effective_sum = WC_VRPayment_Helper::instance()->round_amount( $amount, $currency );
		...
		foreach ( $line_items as $line_item ) {
			if ( $line_item->getType() == \VRPayment\Sdk\Model\LineItemType::DISCOUNT ) {
				...
				$line_item->setAmountIncludingTax( WC_VRPayment_Helper::instance()->round_amount( $item_amount, $currency ) );
			}
		}

		// if there is another difference, like a penny, create a new line item for adjustment.
		$amount_rounded = WC_VRPayment_Helper::instance()->get_total_amount_including_tax( $line_items, $exclude_discounts );
		if ( ! self::compare_numbers( $expected_sum, $amount_rounded ) ) {
			...
			$line_item->setAmountIncludingTax( WC_VRPayment_Helper::instance()->round_amount( $amount_mismatch, $currency ) );
			$line_item->setName( esc_html__( 'Coupon adjustment', 'woo-vrpayment' ) );
...
WC_VRPayment_Packages_Coupon_Discount::init();
