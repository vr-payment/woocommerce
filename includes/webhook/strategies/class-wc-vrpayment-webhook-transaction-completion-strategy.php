...
/**
 * Class WC_VRPayment_Webhook_Transaction_Completion_Strategy
 *
 ...
 */
class WC_VRPayment_Webhook_Transaction_Completion_Strategy extends WC_VRPayment_Webhook_Strategy_Base {
	...
	public function match( string $webhook_entity_id ) {
		return WC_VRPayment_Service_Webhook::VRPAYMENT_TRANSACTION_COMPLETION == $webhook_entity_id;
	}
	...
	public function get_order_id( $object ) {
		/* @var \VRPayment\Sdk\Model\TransactionCompletion $object */
	 ...
	 * @param \VRPayment\Sdk\Model\TransactionCompletion $completion The transaction completion object.
	 * @param WC_VRPayment_Webhook_Request $request The webhook request.
	 * @return void
	 */
	public function bridge_process_order_related_inner( WC_Order $order, \VRPayment\Sdk\Model\TransactionCompletion $completion, WC_VRPayment_Webhook_Request $request ) {
	 ...
	 * @return void
	 */
	protected function process_order_related_inner( WC_Order $order, \VRPayment\Sdk\Model\TransactionCompletion $completion, WC_VRPayment_Webhook_Request $request, $legacy_mode = false ) {
	 ...
	 * @param \VRPayment\Sdk\Model\TransactionCompletion $completion The transaction completion data.
	 * @return void
	 */
	protected function success( WC_Order $order, \VRPayment\Sdk\Model\TransactionCompletion $completion ) {
		$completion_job = WC_VRPayment_Entity_Completion_Job::load_by_completion( $completion->getLinkedSpaceId(), $completion->getId() );
		if ( ! $completion_job->get_id() ) {
			...
			$completion_job = WC_VRPayment_Entity_Completion_Job::load_running_completion_for_transaction(
				...
				if ( $item->is_type( 'line_item' ) && $product && $product->managing_stock() ) {
					...
					if ( $changed_qty > 0 ) {
						...
						$order->add_order_note(
							...
							sprintf( esc_html__( '%1$s stock increased from %2$s to %3$s.', 'woo-vrpayment' ), $item_name, $old_stock, $new_stock )
						);
						do_action( 'wc_vrpayment_restock_not_completed_item', $product->get_id(), $old_stock, $new_stock, $order, $product );
					}
				}
			}
		}
	 ...
	 *
	 * @param WC_Order $order The associated WooCommerce order.
	 * @param \VRPayment\Sdk\Model\TransactionCompletion $completion The transaction completion data that failed.
	 * @return void
	 */
	protected function failed( WC_Order $order, \VRPayment\Sdk\Model\TransactionCompletion $completion ) {
		$completion_job = WC_VRPayment_Entity_Completion_Job::load_by_completion( $completion->getLinkedSpaceId(), $completion->getId() );
		if ( ! $completion_job->get_id() ) {
			$completion_job = WC_VRPayment_Entity_Completion_Job::load_running_completion_for_transaction(
				$completion->getLinkedSpaceId(),
