...
class WC_VRPayment_Webhook_Transaction_Completion_Strategy extends WC_VRPayment_Webhook_Strategy_Base {
	...
	protected function load_entity( WC_VRPayment_Webhook_Request $request ) {
		$transaction_invoice_service = new \VRPayment\Sdk\Service\TransactionCompletionService( WC_VRPayment_Helper::instance()->get_api_client() );
	 ...
	 * @return void
	 */
	protected function process_order_related_inner( WC_Order $order, \VRPayment\Sdk\Model\TransactionCompletion $completion, WC_VRPayment_Webhook_Request $request ) {
				...
				if ( $item->is_type( 'line_item' ) && $product && $product->managing_stock() ) {
					...
					if ( $changed_qty > 0 ) {
						...
						$order->add_order_note(
							...
							sprintf( __( '%1$s stock increased from %2$s to %3$s.', 'woo-vrpayment' ), $item_name, $old_stock, $new_stock )
						);
