...
class WC_VRPayment_Webhook_Transaction_Completion extends WC_VRPayment_Webhook_Order_Related_Abstract {
	 ...
	 * Canonical processor.
	 *
	 * @var WC_VRPayment_Webhook_Transaction_Completion_Strategy
	 */
	private $strategy;
	...
	public function __construct() {
		$this->strategy = new WC_VRPayment_Webhook_Transaction_Completion_Strategy();
	}
	...
	protected function load_entity( WC_VRPayment_Webhook_Request $request ) {
		wc_deprecated_function(
            __METHOD__,
            '3.0.12',
            'WC_VRPayment_Webhook_Transaction_Completion_Strategy::load_entity'
        );
	...
	protected function get_order_id( $completion ) {
		wc_deprecated_function(
            __METHOD__,
            '3.0.12',
            'WC_VRPayment_Webhook_Transaction_Completion_Strategy::get_order_id'
        );
	...
	protected function process_order_related_inner( WC_Order $order, $completion, $request ) {
		wc_deprecated_function(
            __METHOD__,
            '3.0.12',
            'WC_VRPayment_Webhook_Transaction_Completion_Strategy::process_order_related_inner'
        );
