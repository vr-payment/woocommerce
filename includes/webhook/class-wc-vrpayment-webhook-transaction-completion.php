 ...
 * @see WC_VRPayment_Webhook_Transaction_Completion_Strategy
 */
class WC_VRPayment_Webhook_Transaction_Completion extends WC_VRPayment_Webhook_Order_Related_Abstract {
	 ...
	 * @return object|\VRPayment\Sdk\Model\TransactionCompletion TransactionCompletion.
	...
	protected function get_order_id( $completion ) {
		/* @var \VRPayment\Sdk\Model\TransactionCompletion $completion */ //phpcs:ignore
		return WC_VRPayment_Entity_Transaction_Info::load_by_transaction( $completion->getLineItemVersion()->getTransaction()->getLinkedSpaceId(), $completion->getLineItemVersion()->getTransaction()->getId() )->get_order_id();
	}
	 ...
	 * Success.
	 *
	 * @param \VRPayment\Sdk\Model\TransactionCompletion $completion completion.
	 * @param WC_Order $order order.
	 * @return void
	 * @throws Exception Exception.
	 */
	protected function success( \VRPayment\Sdk\Model\TransactionCompletion $completion, WC_Order $order ) {
	 ...
	 * @throws Exception Exception.
	 */
	protected function failed( \VRPayment\Sdk\Model\TransactionCompletion $completion, WC_Order $order ) {
		$completion_job = WC_VRPayment_Entity_Completion_Job::load_by_completion( $completion->getLinkedSpaceId(), $completion->getId() );
		if ( ! $completion_job->get_id() ) {
			$completion_job = WC_VRPayment_Entity_Completion_Job::load_running_completion_for_transaction(
				$completion->getLinkedSpaceId(),
