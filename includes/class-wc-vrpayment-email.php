...
class WC_VRPayment_Email {
	...
	public static function init() {
		add_action(
			'vrpayment_transaction_authorized_send_email',
			array(
				__CLASS__,
	...
	public static function send_on_hold_email_when_authorized( $order_id ) {
		...
		self::send_on_hold_email( $order, '_vrpayment_on_hold_email_sent', true );
	}
	...
	private static function get_transaction_mapped_status( string $transaction_status ) {
		$status = apply_filters( 'vrpayment_wc_status_for_transaction', $transaction_status );
		if ( empty( $status ) ) {
			$defaults = apply_filters( 'vrpayment_default_order_status_mappings', array() );
	...
	public static function send_email_for_order( $enabled, $order ) {
		...
		if ( isset( $GLOBALS['vrpayment_resend_email'] ) && $GLOBALS['vrpayment_resend_email'] ) {
			return $enabled;
		}
		$gateway = wc_get_payment_gateway_by_order( $order );
		if ( $gateway instanceof WC_VRPayment_Gateway ) {
			$send = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_SHOP_EMAIL, 'yes' );
			if ( 'yes' !== $send ) {
				return;
			}
	...
	public static function before_resend_email( $order ) { //phpcs:ignore
		$GLOBALS['vrpayment_resend_email'] = true;
	}
	...
	public static function after_resend_email( $order, $email ) { //phpcs:ignore
		unset( $GLOBALS['vrpayment_resend_email'] );
	}
	...
	$to_add = array(
		'woocommerce_order_status_vrpaym-redirected_to_processing',
		'woocommerce_order_status_vrpaym-redirected_to_completed',
		'woocommerce_order_status_vrpaym-redirected_to_on-hold',
		'woocommerce_order_status_vrpaym-redirected_to_vrpayment-waiting',
		'woocommerce_order_status_vrpaym-redirected_to_vrpayment-manual',
		'woocommerce_order_status_vrpaym-manual_to_cancelled',
		'woocommerce_order_status_vrpaym-waiting_to_cancelled',
		'woocommerce_order_status_vrpaym-manual_to_processing',
		'woocommerce_order_status_vrpaym-waiting_to_processing',
	);
		...
		if ( class_exists( 'woocommerce_wpml' ) ) {
			...
			if ( ! is_null( $woocommerce_wpml ) ) { //phpcs:ignore
				...
				$notifications_all = array(
					'woocommerce_order_status_vrpaym-redirected_to_processing_notification',
					'woocommerce_order_status_vrpaym-redirected_to_completed_notification',
					'woocommerce_order_status_vrpaym-redirected_to_on-hold_notification',
					'woocommerce_order_status_vrpaym-redirected_to_vrpayment-waiting_notification',
					'woocommerce_order_status_vrpaym-redirected_to_vrpayment-manual_notification',
				);
				$notifications_customer = array(
					'woocommerce_order_status_vrpaym-manual_to_processing_notification',
					'woocommerce_order_status_vrpaym-waiting_to_processing_notification',
					'woocommerce_order_status_on-hold_to_processing_notification',
					'woocommerce_order_status_vrpaym-manual_to_cancelled_notification',
					'woocommerce_order_status_vrpaym-waiting_to_cancelled_notifcation',
				);
			...
			add_filter(
				...
				function ( $actions ) use ( $manual_status, $processing_status ) {
					...
					if ( $manual_status && $processing_status ) {
						WC_VRPayment_Email::add_unique_action( $all, 'woocommerce_order_status_' . $processing_status . '_to_' . $manual_status );
						WC_VRPayment_Email::add_unique_action( $all, 'woocommerce_order_status_' . $processing_status . '_to_' . $manual_status . '_notification' );
						WC_VRPayment_Email::add_unique_action( $all, 'woocommerce_order_status_' . $manual_status . '_to_' . $processing_status );
						WC_VRPayment_Email::add_unique_action( $all, 'woocommerce_order_status_' . $manual_status . '_to_' . $processing_status . '_notification' );

						WC_VRPayment_Email::add_unique_action( $customers, 'woocommerce_order_status_' . $processing_status . '_to_' . $manual_status . '_notification' );
						WC_VRPayment_Email::add_unique_action( $customers, 'woocommerce_order_status_' . $manual_status . '_to_' . $processing_status . '_notification' );
					}
	...
	public static function add_email_classes( $emails ) {
		...
		if ( isset( $emails['WC_GZD_Email_Customer_Paid_For_Order'] ) ) {
			...
			add_action( 'woocommerce_order_status_vrpaym-redirected_to_processing_notification', array( $email_object, 'trigger' ), 10, 2 );
			add_action( 'woocommerce_order_status_vrpaym-manual_to_processing_notification', array( $email_object, 'trigger' ), 10, 2 );
			add_action( 'woocommerce_order_status_vrpaym-waiting_to_processing_notification', array( $email_object, 'trigger' ), 10, 2 );
			if ( $authorized_status && $fulfill_status ) {
		...
		foreach ( $emails as $key => $email_object ) {
			switch ( $key ) {
				case 'WC_Email_New_Order':
					add_action( 'woocommerce_order_status_vrpaym-redirected_to_processing_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_vrpaym-redirected_to_completed_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_vrpaym-redirected_to_on-hold_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_vrpaym-redirected_to_vrpayment-waiting_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_vrpaym-redirected_to_vrpayment-manual_notification', array( $email_object, 'trigger' ), 10, 2 );

					break;

				case 'WC_Email_Cancelled_Order':
					add_action( 'woocommerce_order_status_vrpaym-manual_to_cancelled_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_vrpaym-waiting_to_cancelled_notification', array( $email_object, 'trigger' ), 10, 2 );
					break;
     ...
     * when the order was placed using any VRPayment payment method
    ...
    public static function disable_pending_payment_email( $enabled, $order ) {
        ...
        $disable = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_DISABLE_PENDING_EMAIL, 'no' );
        $gateway = wc_get_payment_gateway_by_order( $order );

        if ( $gateway instanceof WC_VRPayment_Gateway && $disable === 'yes' ) {
            return false;
        }
