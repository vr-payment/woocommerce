 ...
 */
final class WC_VRPayment_Blocks_Support extends AbstractPaymentMethodType {
	/**
	 * Payment method name defined by payment methods extending this class.
	 *
	 * @var string
	 */
	protected $name = 'vrpayment';

	/**
	 * Initializes the payment method type.
	 */
	public function initialize() {
		$this->settings['space_id'] = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_SPACE_ID );
	}
	...
	public function get_payment_method_script_handles() {
		...
		wp_register_script(
			...
			WooCommerce_VRPayment::instance()->plugin_url() . '/assets/js/frontend/blocks/build/index.js',
			$dependencies,
			$version,
			true
		);
		...
		return array( 'WooCommerce_VRPayment_blocks_support' );
	}
	...
	public function get_payment_method_data() {
		return array(
			'title' => 'VRPayment',
			'description' => 'VRPayment description',
		);
	}
	...
	public static function get_payment_methods(): array {
		try {
			...
			$available_payment_methods = WC_VRPayment_Service_Transaction::instance()->get_possible_payment_methods_for_cart();

			$payment_plugin = array_filter(
			  $payment_gateways,
			  fn( $key ) => str_contains( $key, 'vrpayment_' ),
			...
			$payments_list =
			  array_map(
				function ( $payment_gateway ) use ( $available_payment_methods ) {
					$has_subscription = WC_VRPayment_Zero_Gateway::cart_has_subscription();
					$cartTotal = (WC()->cart && WC()->cart->total) ?? 0;

					$isPaymentMethodVisibleOnCheckout = $payment_gateway->get_payment_configuration_id() === WC_VRPayment_Zero_Gateway::ZERO_PAYMENT_CONF_ID && $cartTotal == 0;
					$integration_mode = $payment_gateway->get_payment_configuration_id() === WC_VRPayment_Zero_Gateway::ZERO_PAYMENT_CONF_ID
						? WC_VRPayment_Integration::VRPAYMENT_PAYMENTPAGE
						: get_option( WooCommerce_VRPayment::VRPAYMENT_CK_INTEGRATION );
	...
	public static function get_payment_methods_json() {
		if ( ! isset( $_POST['vrpayment_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['vrpayment_nonce'] ) ), 'vrpayment_nonce_block' ) ) {  //phpcs:ignore
	...
	public static function enqueue_portal_scripts() {
		...
		try {
			...
			switch( get_option( WooCommerce_VRPayment::VRPAYMENT_CK_INTEGRATION ) ) {
				case WC_VRPayment_Integration::VRPAYMENT_IFRAME:
					$js_url = $transaction_service->get_javascript_url_for_transaction( $transaction );
					break;
				case WC_VRPayment_Integration::VRPAYMENT_LIGHTBOX:
	...
	public static function process_payment( PaymentContext $context, PaymentResult &$result ) {
		...
		if ( ! $payment_method_object instanceof \WC_VRPayment_Gateway ) {
			return;
		}
		...
		$transaction_service->api_client->addDefaultHeader(
			...
			WC_VRPayment_Helper::VRPAYMENT_CHECKOUT_TYPE_BLOCKS
		);
