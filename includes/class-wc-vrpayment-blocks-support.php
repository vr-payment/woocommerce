...
final class WC_VRPayment_Blocks_Support extends AbstractPaymentMethodType {
	...
	public static function get_payment_methods(): array {
		...
		$payment_plugin = array_filter( $payment_gateways, fn( $key ) => str_contains( $key, 'vrpayment_' ), ARRAY_FILTER_USE_KEY );

		// Build the list with the keys expected by WooCommerce Blocks registering functionality.
		$payments_list = array_map(
			function ( WC_VRPayment_Gateway $payment_gateway ) {
				return array(
					...
					'integration_mode' => ( get_option( WooCommerce_VRPayment::VRPAYMENT_CK_INTEGRATION ) == WC_VRPayment_Integration::VRPAYMENT_LIGHTBOX ) ? 'lightbox' : 'iframe',
	...
	public static function get_payment_methods_json() {
		if ( ! isset( $_POST['vrpayment_nonce'] ) || ! wp_verify_nonce( $_POST['vrpayment_nonce'], 'vrpayment_nonce_block' ) ) {  //phpcs:ignore
	...
	public static function enqueue_portal_scripts() {
		...
		try {
			...
			if ( get_option( WooCommerce_VRPayment::VRPAYMENT_CK_INTEGRATION ) == WC_VRPayment_Integration::VRPAYMENT_LIGHTBOX ) {
