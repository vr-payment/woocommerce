...
/**
 * WC_VRPayment_Helper Class.
 */
class WC_VRPayment_Helper {

	const VRPAYMENT_SHOP_SYSTEM = 'x-meta-shop-system';
	const VRPAYMENT_SHOP_SYSTEM_VERSION = 'x-meta-shop-system-version';
	const VRPAYMENT_SHOP_SYSTEM_AND_VERSION = 'x-meta-shop-system-and-version';
	const VRPAYMENT_CHECKOUT_VERSION = 'x-checkout-type';
	const VRPAYMENT_CHECKOUT_TYPE_BLOCKS = 'blocks';
	const VRPAYMENT_CHECKOUT_TYPE_LEGACY = 'legacy';
	const VRPAYMENT_PLUGIN_VERSION = 'x-meta-plugin-version';
	...
	public static function is_custom_status_mapping_enabled() {
		$enabled = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_ENABLE_CUSTOM_STATUS_MAPPING, 'no' );
		return 'yes' === apply_filters( 'vrpayment_enable_custom_status_mapping_value', $enabled );
	}
	...
	public static function map_status_to_current_mode( $status_slug, $prefixed = false ) {
		...
		$fallback_statuses = array(
			'vrpaym-manual' => 'on-hold',
			'vrpaym-redirected' => 'processing',
			'vrpaym-waiting' => 'processing',
		);
	...
	public function delete_provider_transients() {
		$transients = array(
			'wc_vrpayment_currencies',
			'wc_vrpayment_label_description_groups',
			'wc_vrpayment_label_descriptions',
			'wc_vrpayment_languages',
			'wc_vrpayment_payment_connectors',
			'wc_vrpayment_payment_methods',
		);
	...
	public function get_api_client() {
		if ( null === $this->api_client ) {
			...
			if ( ! empty( $user_id ) && ! empty( $user_key ) ) {
				$this->api_client = new \VRPayment\Sdk\ApiClient( $user_id, $user_key );
				$this->api_client->setBasePath( rtrim( $this->get_base_gateway_url(), '/' ) . '/api' );
				self::add_headers( $this->api_client );
			} else {
				throw new Exception( esc_html__( 'The API access data is incomplete.', 'woo-vrpayment' ) );
			}
		}
	...
	public function get_base_gateway_url() {
		return get_option( 'wc_vrpayment_base_gateway_url', 'https://gateway.vr-payment.de/' );
	}
	...
	public function translate( $translated_string, $language = null ) {
		...
		try {
			$language_provider = WC_VRPayment_Provider_Language::instance();
	...
	/**
	 * Returns the URL to a resource on VRPayment in the given context (space, space view, language).
	 *
	...
	public function get_currency_fraction_digits( $currency_code ) {
		$currency_provider = WC_VRPayment_Provider_Currency::instance();
	...
	public function get_total_amount_including_tax( array $line_items, bool $exclude_discounts = false ) {
		...
		foreach ( $line_items as $line_item ) {
			...
			if ( $exclude_discounts && \VRPayment\Sdk\Model\LineItemType::DISCOUNT === $type
				&& strpos( $name, WC_VRPayment_Packages_Coupon_Discount::VRPAYMENT_COUPON ) !== false
			) {
			...
			elseif ( \VRPayment\Sdk\Model\LineItemType::DISCOUNT === $type
				&& stripos( $name, WC_VRPayment_Packages_Gift_Card::VRPAYMENT_GIFT_CARD) !== false
			) {
	...
	public function cleanup_line_items( array $line_items, $expected_sum, $currency, bool $is_recurrent = false ) {
		...
		if ( $is_recurrent ) {
			$has_coupons = apply_filters( 'wc_vrpayment_packages_coupon_line_items_have_coupon_discounts', $line_items, $currency );
		} else {
			$has_coupons = apply_filters( 'wc_vrpayment_packages_coupon_cart_has_coupon_discounts_applied', $currency ); //phpcs:ignore
		}
		...
		if ( $has_coupons ) {
			$result = apply_filters( 'wc_vrpayment_packages_coupon_process_line_items_with_coupons', $line_items, $expected_sum, $currency ); //phpcs:ignore
		...
		if ( 0 !== (int) $inconsistent_amount ) {
			$enforce_consistency = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_ENFORCE_CONSISTENCY );
			switch ( $enforce_consistency ) {
				case 'no':
					...
					$line_item->setName( esc_html__( 'Adjustment', 'woo-vrpayment' ) );
					$line_item->setQuantity( 1 );
					$line_item->setSku( 'adjustment' );
					$line_item->setUniqueId( 'adjustment' );
					$line_item->setShippingRequired( false );
					$line_item->setType( $enforce_consistency > 0 ? \VRPayment\Sdk\Model\LineItemType::FEE : \VRPayment\Sdk\Model\LineItemType::DISCOUNT );
					$line_items[] = $line_item;
					break;
				default:
					throw new WC_VRPayment_Exception_Invalid_Transaction_Amount( esc_html( $effective_sum ), esc_html( $rounded_expected_sum ) );
			}
		}
	...
	public function get_current_cart_id() {
		...
		$current_cart_id = $session_handler->get( 'vrpayment_current_cart_id', null );
		if ( null === $current_cart_id ) {
			$current_cart_id = WC_VRPayment_Unique_Id::get_uuid();
			$session_handler->set( 'vrpayment_current_cart_id', $current_cart_id );
		}
	...
	public function destroy_current_cart_id() {
		$session_handler = WC()->session;
		$session_handler->set( 'vrpayment_current_cart_id', null );
	}
	...
	public function get_clean_locale_for_string( $language_string, $use_default ) {
		...
		if ( strlen( $language_string ) >= 5 ) {
			...
			$language = WC_VRPayment_Provider_Language::instance()->find( $language_string );
			if ( ! $language && strpos( $language_string, '-' ) !== false ) {
				...
				while ( ! $language && ! empty( $language_parts ) ) {
					$language = WC_VRPayment_Provider_Language::instance()->find( implode( '-', $language_parts ) );
					array_pop( $language_parts );
				}
			}
		}
		if ( ! $language ) {
			...
			$language = WC_VRPayment_Provider_Language::instance()->find_by_iso_code( $language_string );
		}
	...
	public function try_to_parse_date( $date_string ) {
		$date_of_birth = false;
		$custom_date_of_birth_format = apply_filters( 'wc_vrpayment_custom_date_of_birth_format', '' ); //phpcs:ignore
	...
	protected function restock_items_for_order( WC_Order $order ) {
		if (
			...
			&& $order && apply_filters( 'vrpayment_can_increase_order_stock', true, $order )//phpcs:ignore
			...
			foreach ( $order->get_items() as $item ) {
				...
				if ( $item->is_type( 'line_item' ) && $product && $product->managing_stock() ) {
					...
					if ( ! is_wp_error( $new_stock ) ) {
						...
						$order->add_order_note( sprintf( esc_html__( '%1$s stock increased from %2$s to %3$s.', 'woo-vrpayment' ), $item_name, $new_stock - $qty, $new_stock ) );
					}
				}
			}
			do_action( 'wc_vrpayment_restocked_order', $order ); //phpcs:ignore
		}
	}
	...
	protected static function get_default_header_data() {
		...
		return array(
			...
			self::VRPAYMENT_SHOP_SYSTEM_VERSION => $shop_version,
			self::VRPAYMENT_SHOP_SYSTEM_AND_VERSION => 'woocommerce-' . $major_version . '.' . $minor_version,
			self::VRPAYMENT_PLUGIN_VERSION => $plugin_version,
		);
	}
	...
	public static function update_order_status_for_preorder_if_needed( $order ) {
		...
		if ( $preorder_status_slug ) {
			...
			$order->update_status(
			  ...
			  __( 'Product is on pre-order. Status set automatically.', 'woo-vrpayment' )
			);

			$order->add_order_note(
			  __( 'Order status automatically set to pre-order.', 'woo-vrpayment' )
			);
		}
	}

	/**
	 * Get default VRPayment settings array
	 *
	 * @return array
	 */
	public function get_default_settings() {
		$settings = array(
			WooCommerce_VRPayment::VRPAYMENT_CK_SHOP_EMAIL => 'yes',
			WooCommerce_VRPayment::VRPAYMENT_CK_CUSTOMER_INVOICE => 'yes',
			WooCommerce_VRPayment::VRPAYMENT_CK_CUSTOMER_PACKING => 'yes',
			WooCommerce_VRPayment::VRPAYMENT_CK_INTEGRATION => WC_VRPayment_Integration::VRPAYMENT_PAYMENTPAGE,
			WooCommerce_VRPayment::VRPAYMENT_CK_ENFORCE_CONSISTENCY => 'yes',
			WooCommerce_VRPayment::VRPAYMENT_CK_ORDER_REFERENCE => WC_VRPayment_Order_Reference::VRPAYMENT_ORDER_ID,
			WooCommerce_VRPayment::VRPAYMENT_CK_CHANGE_ORDER_STATUS => 'yes',
		);
		return $settings;
	}
