 ...
 */
class WC_VRPayment_Service_Line_Item extends WC_VRPayment_Service_Abstract {
	...
	public function get_items_from_session() {
		...
		return WC_VRPayment_Helper::instance()->cleanup_line_items( $combined, $cart->total, $currency );
	}
	...
	private function translate_item_language( $items ) {
		...
		if ( isset( $GLOBALS['TRP_LANGUAGE'] ) ) {
			$vrpayment_trp_language = $GLOBALS['TRP_LANGUAGE'];
		}
		...
		$dictionary = $query->get_string_rows( '', $strings, $vrpayment_trp_language ); //phpcs:ignore
	...
	protected function create_product_line_items_from_session( WC_Cart $cart, $currency ) {
		...
		foreach ( $cart->get_cart() as $values ) {
			...
			$items[] = apply_filters( 'wc_vrpayment_modify_line_item_product_session', $this->clean_line_item( $line_item ), $values ); //phpcs:ignore
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_session( WC_Cart $cart, $currency ) {
		...
		foreach ( $cart->get_fees() as $fee ) {
			...
			if ( $amount_including_tax < 0 ) {
				...
				$line_item->setType( \VRPayment\Sdk\Model\LineItemType::DISCOUNT );
				$line_item->setUniqueId( 'discount-' . $fee->id );
			} else {
				$line_item->setType( \VRPayment\Sdk\Model\LineItemType::FEE );
				$line_item->setUniqueId( 'fee-' . $fee->id );
			}
			$fees[] = apply_filters( 'wc_vrpayment_modify_line_item_fee_session', $this->clean_line_item( $line_item ), $fee ); //phpcs:ignore
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_session( $packages, $chosen_methods, $currency ) {
		...
		foreach ( $packages as $package_key => $package ) {
			if ( isset( $chosen_methods[ $package_key ], $package['rates'][ $chosen_methods[ $package_key ] ] ) ) {
				...
				$line_item->setType( \VRPayment\Sdk\Model\LineItemType::SHIPPING );
				$line_item->setUniqueId( WC_VRPayment_Unique_Id::get_uuid() );

				$shippings[] = apply_filters( 'wc_vrpayment_modify_line_item_shipping_session', $this->clean_line_item( $line_item ), $shipping_rate ); //phpcs:ignore
			}
		}
		return $shippings;
	}
	 ...
	 * @return \VRPayment\Sdk\Model\LineItemCreate|null The line item created or null if the coupon is not valid.
	 */
	private function create_coupon_line_items( $coupon, float $total_discount_amount = 0, array $items = [] ) {
		...
		foreach ( $discounts as $discount ) {
			...
			$line_item->setName( sprintf( '%s: %s (%s%% tax)', WC_VRPayment_Packages_Coupon_Discount::VRPAYMENT_COUPON, $coupon->get_code(), $discount['rate_id'] ) );
			$line_item->setQuantity( 1 );
			$line_item->setShippingRequired( false );
			$line_item->setSku( $sku, 200 );
			$line_item->setType( \VRPayment\Sdk\Model\LineItemType::DISCOUNT );
			...
			$tax_rate = new \VRPayment\Sdk\Model\TaxCreate(
				array(
	...
	public function get_items_from_order( WC_Order $order ) {
		...
		$items = apply_filters( 'wc_vrpayment_modify_line_item_order', $raw, $order ); //phpcs:ignore
		$total = apply_filters( 'wc_vrpayment_modify_total_to_check_order', $order->get_total(), $order ); //phpcs:ignore
		return WC_VRPayment_Helper::instance()->cleanup_line_items( $items, $total, $order->get_currency() );
	}
	...
	protected function create_product_line_items_from_order( WC_Order $order ) {
		...
		foreach ( $order_items as $item ) {
			...
			$line_item->setType( \VRPayment\Sdk\Model\LineItemType::PRODUCT );
			$line_item->setUniqueId( $item->get_meta( '_vrpayment_unique_line_item_id', true ) );
			...
			$items[] = apply_filters( 'wc_vrpayment_modify_line_item_product_order', $this->clean_line_item( $line_item ), $item ); //phpcs:ignore
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_order( WC_Order $order ) {
		...
		foreach ( $order->get_fees() as $fee ) {
			...
			$line_item->setUniqueId( $fee->get_meta( '_vrpayment_unique_line_item_id', true ) );

			$fees[] = apply_filters( 'wc_vrpayment_modify_line_item_fee_order', $this->clean_line_item( $line_item ), $fee ); //phpcs:ignore
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_order( WC_Order $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping ) {
			...
			$line_item->setType( \VRPayment\Sdk\Model\LineItemType::SHIPPING );
			$line_item->setUniqueId( $shipping->get_meta( '_vrpayment_unique_line_item_id', true ) );

			$shippings[] = apply_filters( 'wc_vrpayment_modify_line_item_shipping_order', $this->clean_line_item( $line_item ), $shipping ); //phpcs:ignore
		}
		return $shippings;
	}
	...
	public function get_items_from_backend( array $backend_items, $amount, WC_Order $order ) {
		...
		return WC_VRPayment_Helper::instance()->cleanup_line_items( $combined, $amount, $order->get_currency() );
	}
	...
	protected function create_product_line_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$item_data_coupon = $item->get_meta( '_vrpayment_coupon_discount_line_item_discounts' );
			...
			$items[] = apply_filters( 'wc_vrpayment_modify_line_item_product_backend', $this->clean_line_item( $line_item ), $item ); //phpcs:ignore
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_fees() as $fee_id => $fee ) {
			...
			$fees[] = apply_filters( 'wc_vrpayment_modify_line_item_fee_backend', $this->clean_line_item( $line_item ), $fee ); //phpcs:ignore
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping_id => $shipping ) {
			...
			$shippings[] = apply_filters( 'wc_vrpayment_modify_line_item_shipping_backend', $this->clean_line_item( $line_item ), $shipping ); //phpcs:ignore
		}
		return $shippings;
	}
	 ...
	 *
	 * @param \VRPayment\Sdk\Model\LineItemCreate $line_item line item.
	 * @return \VRPayment\Sdk\Model\LineItemCreate
	 */
	protected function clean_line_item( \VRPayment\Sdk\Model\LineItemCreate $line_item ) {
	...
	protected function get_taxes( array $rates_or_ids ) {
		...
		foreach ( array_keys( $rates_or_ids ) as $rate_id ) {
			$tax_rates[] = new \VRPayment\Sdk\Model\TaxCreate(
				array(
	...
	private function get_base_attributes( $product_id ) {
		...
		foreach ( $products_to_check as $id ) {
			...
			if ( $wpm_gtin_code ) {
				$wpm_gtin_attribute = new \VRPayment\Sdk\Model\LineItemAttributeCreate();
			...
			foreach ( $product_attributes as $key => $object ) {
				if ( is_a( $object, 'WC_Product_Attribute' ) ) {
					if ( $object->is_taxonomy() ) {
						$attribute_options = WC_VRPayment_Entity_Attribute_Options::load_by_attribute_id( $object->get_id() );
						if ( $attribute_options->get_send() ) {
							$attribute = new \VRPayment\Sdk\Model\LineItemAttributeCreate();
	 ...
	 * @throws WC_VRPayment_Exception_Invalid_Transaction_Amount WC_VRPayment_Exception_Invalid_Transaction_Amount.
	 */
	public function get_items_for_renewal( WC_Order $order, $order_total, \VRPayment\Sdk\Model\AbstractTransactionPending $transaction ) {
		$raw_items = $this->get_raw_items_from_order( $order );
		return WC_VRPayment_Helper::instance()->cleanup_line_items( $raw_items, $order_total, $order->get_currency(), true );
	}
	...
	protected function create_gift_card_line_items_from_session() {
		...
		foreach ( $redeemed as $item ) {
			...
			$line_item->setName( sprintf( __( 'Gift Card (%s)', 'woo-vrpayment' ), $code ) );
			$line_item->setQuantity( 1 );
			$line_item->setShippingRequired( false );
	...
	protected function create_gift_card_line_items_from_order( $order) {
		...
		foreach ( $items as $giftcard_item ) {
			...
			$line_item = new \VRPayment\Sdk\Model\LineItemCreate();
			// VRPayment expects discounts as negative values
			$line_item->setAmountIncludingTax( -1 * abs( $used_amount ) );
			/* translators: %s: gift card code */
			$line_item->setName( sprintf( __( 'Gift Card (%s)', 'woo-vrpayment' ), $code ) );
			$line_item->setQuantity( 1 );
