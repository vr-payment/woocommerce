...
/**
 * This service provides functions to deal with VRPayment refunds.
 */
class WC_VRPayment_Service_Refund extends WC_VRPayment_Service_Abstract {

	/**
	 * The refund API service.
	 *
	 * @var \VRPayment\Sdk\Service\RefundService refund service.
	 */
	...
	public function create( WC_Order $order, WC_Order_Refund $refund ) {
		...
		$transaction = WC_VRPayment_Service_Transaction::instance()->get_transaction( $transaction_info->get_space_id(), $transaction_info->get_transaction_id() );

		$reductions = $this->get_reductions( $order, $refund );
		$reductions = $this->fix_reductions( $refund, $transaction, $reductions );

		$refund_create = new \VRPayment\Sdk\Model\RefundCreate();
		...
		$refund_create->setType( \VRPayment\Sdk\Model\RefundType::MERCHANT_INITIATED_ONLINE );
	 ...
	 * @param \VRPayment\Sdk\Model\LineItemReductionCreate[] $reductions reductions.
	 * @return \VRPayment\Sdk\Model\LineItemReductionCreate[]
	 * @throws Exception Exception.
	 */
	protected function fix_reductions( WC_Order_Refund $refund, \VRPayment\Sdk\Model\Transaction $transaction, array $reductions ) {
	 ...
	 * @param \VRPayment\Sdk\Model\LineItem[]                $base_line_items base line items.
	...
	private function distribute_rounding_difference(
		...
		if ( 0 != $new_remainder && $index + 1 < count( $reductions ) ) {
			...
			throw new Exception( esc_html__( 'Could not distribute the rounding difference.', 'woo-vrpayment' ) );
		} else {
			return $reductions;
		}
	}
	...
	protected function get_reductions( WC_Order $order, WC_Order_Refund $refund ) {
		...
		foreach ( $refund->get_items() as $item ) {
			...
			$unique_id = $order_item->get_meta( '_vrpayment_unique_line_item_id', true );
		...
		foreach ( $refund->get_fees() as $fee ) {
			...
			$unique_id = $order_fee->get_meta( '_vrpayment_unique_line_item_id', true );
		...
		foreach ( $refund->get_shipping_methods() as $shipping ) {
			...
			$unique_id = $order_shipping->get_meta( '_vrpayment_unique_line_item_id', true );
	 ...
	 *
	 * @param int                                           $space_id space id.
	 * @param \VRPayment\Sdk\Model\RefundCreate $refund refund.
	 * @return \VRPayment\Sdk\Model\Refund
	 * @throws Exception Exception.
	 */
	public function refund( $space_id, \VRPayment\Sdk\Model\RefundCreate $refund ) {
	 ...
	 * @throws Exception Exception.
	 */
	protected function get_base_line_items( \VRPayment\Sdk\Model\Transaction $transaction, \VRPayment\Sdk\Model\Refund $refund = null ) {
	 ...
	 *
	 * @param \VRPayment\Sdk\Model\Transaction $transaction transaction.
	 * @return \VRPayment\Sdk\Model\TransactionInvoice transaction invoice.
	 * @throws \VRPayment\Sdk\ApiException ApiException.
	 */
	protected function get_transaction_invoice( \VRPayment\Sdk\Model\Transaction $transaction ) {
		$query = new \VRPayment\Sdk\Model\EntityQuery();
		...
		$invoice_service = new \VRPayment\Sdk\Service\TransactionInvoiceService( WC_VRPayment_Helper::instance()->get_api_client() );
	 ...
	 *
	 * @param \VRPayment\Sdk\Model\Transaction $transaction transaction.
	 * @param \VRPayment\Sdk\Model\Refund|null $refund refund.
	 * @return false|\VRPayment\Sdk\Model\Refund
	 * @throws \VRPayment\Sdk\ApiException ApiException.
	 */
	protected function get_last_successful_refund( \VRPayment\Sdk\Model\Transaction $transaction, \VRPayment\Sdk\Model\Refund $refund = null ) {
		$query = new \VRPayment\Sdk\Model\EntityQuery();

		$filter = new \VRPayment\Sdk\Model\EntityQueryFilter();
		$filter->setType( \VRPayment\Sdk\Model\EntityQueryFilterType::_AND );
		$filters = array(
			$this->create_entity_filter( 'state', \VRPayment\Sdk\Model\RefundState::SUCCESSFUL ),
			$this->create_entity_filter( 'transaction.id', $transaction->getId() ),
		);
		if ( null != $refund ) {
			$filters[] = $this->create_entity_filter( 'id', $refund->getId(), \VRPayment\Sdk\Model\CriteriaOperator::NOT_EQUALS );
		}
		...
		$query->setOrderBys(
			array(
				$this->create_entity_order_by( 'createdOn', \VRPayment\Sdk\Model\EntityQueryOrderByType::DESC ),
			)
		);
	...
	protected function get_refund_service() {
		if ( null == $this->refund_service ) {
			$this->refund_service = new \VRPayment\Sdk\Service\RefundService( WC_VRPayment_Helper::instance()->get_api_client() );
		}
