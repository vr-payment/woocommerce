...
class WC_VRPayment_Service_Transaction extends WC_VRPayment_Service_Abstract {
	 ...
	 *
	 * @var \VRPayment\Sdk\Service\TransactionLightboxService
	 */
	...
	{
		$this->api_client = WC_VRPayment_Helper::instance()->get_api_client();
	}
	...
	protected function get_transaction_service() {
		if ( is_null( $this->transaction_service ) ) {
			$this->transaction_service = new \VRPayment\Sdk\Service\TransactionService( $this->api_client );
		}
	...
	protected function get_transaction_iframe_service() {
		if ( is_null( $this->transaction_iframe_service ) ) {
			$this->transaction_iframe_service = new \VRPayment\Sdk\Service\TransactionIframeService( $this->api_client );
		}
	...
	protected function get_transaction_lightbox_service() {
		if ( is_null( $this->transaction_lightbox_service ) ) {
			$this->transaction_lightbox_service = new \VRPayment\Sdk\Service\TransactionLightboxService( $this->api_client );
		}
	...
	protected function get_transaction_payment_page_service() {
		if ( is_null( $this->transaction_payment_page_service ) ) {
			$this->transaction_payment_page_service = new \VRPayment\Sdk\Service\TransactionPaymentPageService( $this->api_client );
		}
	...
	protected function get_charge_attempt_service() {
		if ( is_null( $this->charge_attempt_service ) ) {
			$this->charge_attempt_service = new \VRPayment\Sdk\Service\ChargeAttemptService( $this->api_client );
		}
	...
	protected function get_transaction_line_item_version_service() {
		if ( is_null( $this->transaction_line_item_version_service ) ) {
			$this->transaction_line_item_version_service = new \VRPayment\Sdk\Service\TransactionLineItemVersionService( $this->api_client );
		}
	 ...
	 */
	public function get_javascript_url_for_transaction( \VRPayment\Sdk\Model\Transaction $transaction ) {
	 ...
	 * @throws Exception Exception.
	 */
	public function get_lightbox_url_for_transaction( \VRPayment\Sdk\Model\Transaction $transaction ) {
	...
	public function get_failed_charge_attempt( $space_id, $transaction_id ) {
		...
		$filter->setChildren(
			array(
				...
				$this->create_entity_filter( 'state', \VRPayment\Sdk\Model\ChargeAttemptState::FAILED ),
			)
		);
	 ...
	 * @param int                                               $space_id space id.
	 * @param int                                               $transaction_id transaction id.
	 * @param \VRPayment\Sdk\Model\LineItemCreate[] $line_items line items.
	...
	public function update_line_items( $space_id, $transaction_id, $line_items ) {
		$line_item_version_create = new \VRPayment\Sdk\Model\TransactionLineItemVersionCreate();
	 ...
	 * @throws Exception Exception.
	 */
	public function update_transaction_info( \VRPayment\Sdk\Model\Transaction $transaction, WC_Order $order ) {
		$info = WC_VRPayment_Entity_Transaction_Info::load_by_transaction( $transaction->getLinkedSpaceId(), $transaction->getId() );
		...
		$info = apply_filters( 'wc_vrpayment_update_transaction_info', $info, $transaction, $order ); //phpcs:ignore
		$info->save();
		return $info;
	}
	 ...
	 * @throws Exception Exception.
	 */
	protected function get_transaction_labels( \VRPayment\Sdk\Model\Transaction $transaction ) {
	 ...
	 * @throws Exception Exception.
	 */
	protected function get_charge_attempt( \VRPayment\Sdk\Model\Transaction $transaction ) {
		$charge_attempt_service = $this->get_charge_attempt_service();
		$query = new \VRPayment\Sdk\Model\EntityQuery();
		$filter = new \VRPayment\Sdk\Model\EntityQueryFilter();
		$filter->setType( \VRPayment\Sdk\Model\EntityQueryFilterType::_AND );
		$filter->setChildren(
			array(
				...
				$this->create_entity_filter( 'state', \VRPayment\Sdk\Model\ChargeAttemptState::SUCCESSFUL ),
			)
		);
		$query->setFilter( $filter );
		$query->setNumberOfEntities( 1 );
	 ...
	 * @return string
	 */
	protected function get_payment_method_image( \VRPayment\Sdk\Model\Transaction $transaction, WC_order $order ) {
		if ( is_null( $transaction->getPaymentConnectorConfiguration() ) ) {
			$method_instance = wc_get_payment_gateway_by_order( $order );
			if ( false != $method_instance && ( $method_instance instanceof WC_VRPayment_Gateway ) ) {
				return WC_VRPayment_Helper::instance()->get_resource_url( $method_instance->get_payment_method_configuration()->get_image_base(), $method_instance->get_payment_method_configuration()->get_image() );
			}
			return null;
		}
	 ...
	 * @return array|int[]|\VRPayment\Sdk\Model\PaymentMethodConfiguration The list of possible payment methods.
	 * @throws WC_VRPayment_Exception_Invalid_Transaction_Amount If the transaction amount is invalid.
	 * @throws \VRPayment\Sdk\Http\ConnectionException If there is an API connection error during the process.
	 */
	private function get_possible_payment_methods( $transaction_source ) {
		$id = ( $transaction_source instanceof WC_Order ) ? $transaction_source->get_id() : WC_VRPayment_Helper::instance()->get_current_cart_id();
		...
		if ( ! isset( self::$possible_payment_method_cache[ $id ] ) || is_null( self::$possible_payment_method_cache[ $id ] ) ) {
			try {
				...
				$integration_method = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_INTEGRATION );
				...
				$method_configuration_service = WC_VRPayment_Service_Method_Configuration::instance();
				...
				WooCommerce_VRPayment::instance()->log( __CLASS__ . ' : ' . __FUNCTION__ . ' : ' . __LINE__ . ' : ' . $last->getMessage(), WC_Log_Levels::ERROR );
	 ...
	 *
	 * @return array|int[]|\VRPayment\Sdk\Model\PaymentMethodConfiguration The list of possible payment methods for the cart.
	 * @throws WC_VRPayment_Exception_Invalid_Transaction_Amount If the transaction amount is invalid.
	 * @throws \VRPayment\Sdk\ApiException If there is an API exception during the process.
	 */
	public function get_possible_payment_methods_for_cart() {
		$paymentConfigurations = $this->get_possible_payment_methods( null );
		$paymentConfigurations[] = WC_VRPayment_Zero_Gateway::ZERO_PAYMENT_CONF_ID;
	 ...
	 * @return array|int[]|\VRPayment\Sdk\Model\PaymentMethodConfiguration The list of possible payment methods for the order.
	 * @throws WC_VRPayment_Exception_Invalid_Transaction_Amount If the transaction amount is invalid.
	 * @throws \VRPayment\Sdk\ApiException If there is an API exception during the process.
	 */
	...
	public function confirm_transaction( $transaction_id, $space_id, WC_Order $order, $method_configuration_id ) {
		...
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				if ( $transaction->getState() !== \VRPayment\Sdk\Model\TransactionState::PENDING ) {
					throw new Exception( esc_html__( 'The checkout expired, please try again.', 'woo-vrpayment' ) );
				}
				$pending_transaction = new \VRPayment\Sdk\Model\TransactionPending();
				...
				$pending_transaction = apply_filters( 'wc_vrpayment_modify_confirm_transaction', $pending_transaction, $order ); //phpcs:ignore
	 ...
	 * @throws WC_VRPayment_Exception_Invalid_Transaction_Amount WC_VRPayment_Exception_Invalid_Transaction_Amount.
	 */
	protected function assemble_order_transaction_data( WC_Order $order, \VRPayment\Sdk\Model\AbstractTransactionPending $transaction, bool $set_line_items = true ) {
		...
		if ( ! empty( $language_string ) ) {
			$language = WC_VRPayment_Helper::instance()->get_clean_locale_for_string( $language_string, false );
		}
		if ( empty( $language ) ) {
	...
	protected function getOrderReference( $order ) {
		$reference_type = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_ORDER_REFERENCE );

		if ( WC_VRPayment_Order_Reference::VRPAYMENT_ORDER_NUMBER == $reference_type ) {
	 ...
	 * @param \VRPayment\Sdk\Model\AbstractTransactionPending $transaction transaction.
	 */
	protected function set_order_return_urls( WC_Order $order, \VRPayment\Sdk\Model\AbstractTransactionPending $transaction ) {
	 ...
	 * @throws WC_VRPayment_Exception_Invalid_Transaction_Amount WC_VRPayment_Exception_Invalid_Transaction_Amount.
	 */
	protected function set_order_line_items( WC_Order $order, \VRPayment\Sdk\Model\AbstractTransactionPending $transaction ) {
		$transaction->setLineItems( WC_VRPayment_Service_Line_Item::instance()->get_items_from_order( $order ) );
	}
	...
	protected function get_order_billing_address( WC_order $order ) {
		...
		$custom_billing_date_of_birth_meta_name = apply_filters( 'wc_vrpayment_billing_date_of_birth_order_meta_name', '' ); //phpcs:ignore
		...
		$gender_string = '';
		$custom_billing_gender_meta_name = apply_filters( 'wc_vrpayment_billing_gender_order_meta_name', '' ); //phpcs:ignore
		...
		return apply_filters( 'wc_vrpayment_modify_order_billing_address', $address, $order ); //phpcs:ignore
	}
	...
	protected function get_order_shipping_address( WC_order $order ) {
		...
		$date_of_birth_string = '';
		$custom_shipping_date_of_birth_meta_name = apply_filters( 'wc_vrpayment_shipping_date_of_birth_order_meta_name', '' ); //phpcs:ignore
		...
		$gender_string = '';
		$custom_shipping_gender_meta_name = apply_filters( 'wc_vrpayment_shipping_gender_order_meta_name', '' ); //phpcs:ignore
		...
		return apply_filters( 'wc_vrpayment_modify_order_shipping_address', $address, $order ); //phpcs:ignore
	}
	...
	public function get_transaction_from_session( $current_cart_id = null) {
		...
		if ( null === $current_cart_id ) {
			$current_cart_id = WC_VRPayment_Helper::instance()->get_current_cart_id();
		}
		if ( ! isset( self::$transaction_cache[ $current_cart_id ] ) || null == self::$transaction_cache[ $current_cart_id ] ) {
			$transaction_id = WC()->session->get( 'vrpayment_transaction_id', null );
			$space_id = WC()->session->get( 'vrpayment_space_id', null );
			$configured_space_id = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_SPACE_ID );
	...
	public function get_transaction_from_order( WC_Order $order ) {
		...
		if ( ! isset( self::$transaction_cache[ $order->get_id() ] ) || is_null( self::$transaction_cache[ $order->get_id() ] ) ) {
			...
			} elseif ( $existing_transaction->get_state() == \VRPayment\Sdk\Model\TransactionState::PENDING ) {
	...
	protected function create_transaction_by_order( WC_Order $order ) {
		...
		$create_transaction = new \VRPayment\Sdk\Model\TransactionCreate();
		$create_transaction->setCustomersPresence( \VRPayment\Sdk\Model\CustomersPresence::VIRTUAL_PRESENT );
		$space_view_id = get_option( WooCommerce_VRPayment::VRPAYMENT_CK_SPACE_VIEW_ID, null );
		if ( ! empty( $space_view_id ) ) {
			$create_transaction->setSpaceViewId( $space_view_id );
		}
		$create_transaction->setAutoConfirmationEnabled( false );
		if ( isset( $_COOKIE['wc_vrpayment_device_id'] ) ) {
			$create_transaction->setDeviceSessionIdentifier( sanitize_text_field( wp_unslash( $_COOKIE['wc_vrpayment_device_id'] ) ) );
		}
		$this->assemble_order_transaction_data( $order, $create_transaction );
		$create_transaction = apply_filters( 'wc_vrpayment_modify_order_create_transaction', $create_transaction, $order ); //phpcs:ignore
	...
	protected function create_transaction_from_session() {
		...
		$create_transaction = apply_filters( 'wc_vrpayment_modify_session_create_transaction', $create_transaction ); //phpcs:ignore
	 ...
	 *
	 * @param WC_Order $order order.
	 * @param WC_VRPayment_Entity_Transaction_Info $existing_transaction existing transaction.
	 * @return \VRPayment\Sdk\Model\Transaction|\VRPayment\Sdk\Model\TransactionCreate
	 * @throws Exception Exception.
	 */
	protected function load_and_update_transaction_for_order( WC_Order $order, WC_VRPayment_Entity_Transaction_Info $existing_transaction ) {
		$last = new \Exception( __FUNCTION__ );
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				$pending_transaction->setVersion( $transaction->getVersion() );
				$this->assemble_order_transaction_data( $order, $pending_transaction );
				$pending_transaction = apply_filters( 'wc_vrpayment_modify_order_pending_transaction', $pending_transaction, $order ); //phpcs:ignore
	...
	public function load_and_update_transaction_from_session() {
		...
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				$space_id = $session_handler->get( 'vrpayment_space_id' );
				$transaction_id = $session_handler->get( 'vrpayment_transaction_id' );
				$transaction = $this->get_transaction( $space_id, $transaction_id );
				if (
					$transaction->getState() != \VRPayment\Sdk\Model\TransactionState::PENDING
				...
				$pending_transaction = apply_filters( 'wc_vrpayment_modify_session_pending_transaction', $pending_transaction ); //phpcs:ignore
	 ...
	 * @param \VRPayment\Sdk\Model\AbstractTransactionPending $transaction transaction.
	 */
	protected function assemble_session_transaction_data( \VRPayment\Sdk\Model\AbstractTransactionPending $transaction ) {
		...
		$transaction->setCustomerId( $this->get_customer_id() );
		$transaction->setLanguage( WC_VRPayment_Helper::instance()->get_cleaned_locale() );
	 ...
	 * @throws WC_VRPayment_Exception_Invalid_Transaction_Amount WC_VRPayment_Exception_Invalid_Transaction_Amount.
	 */
	protected function set_session_line_items( \VRPayment\Sdk\Model\AbstractTransactionPending $transaction ) {
		$transaction->setLineItems( WC_VRPayment_Service_Line_Item::instance()->get_items_from_session() );
	}

	/**
	 * Returns the billing address of the current session.
	 *
	 * @return \VRPayment\Sdk\Model\AddressCreate|null
	 */
	protected function get_session_billing_address() {
		...
		$date_of_birth_string = $customer->get_meta( '_vrpayment_billing_date_of_birth', true, 'edit' );
		$this->set_address_dob( $address, $date_of_birth_string );

		$gender_string = $customer->get_meta( '_vrpayment_billing_gender', true, 'edit' );
		$this->set_address_gender( $address, $gender_string );

		return apply_filters( 'wc_vrpayment_modify_session_billing_address', $address ); //phpcs:ignore
	}
	...
	protected function get_session_shipping_address() {
		...
		$date_of_birth_string = $customer->get_meta( '_vrpayment_shipping_date_of_birth', true, 'edit' );
		$this->set_address_dob( $address, $date_of_birth_string );

		$gender_string = $customer->get_meta( '_vrpayment_shipping_gender', true, 'edit' );
		$this->set_address_gender( $address, $gender_string );

		return apply_filters( 'wc_vrpayment_modify_session_shipping_address', $address ); //phpcs:ignore
	}
	...
	protected function get_session_email_address() {
		...
		$sanitized_post_data = wp_verify_nonce( isset( $_POST['post_data'] ), 'vrpayment_nonce_block' );
	 ...
	 * @param \VRPayment\Sdk\Model\Transaction $transaction transaction.
	 */
	protected function store_transaction_ids_in_session( \VRPayment\Sdk\Model\Transaction $transaction ) {
		$session_handler = WC()->session;
		$session_handler->set( 'vrpayment_transaction_id', $transaction->getId() );
		$session_handler->set( 'vrpayment_space_id', $transaction->getLinkedSpaceId() );
	}

	/**
	 * Set address dob
	 *
	 * @param \VRPayment\Sdk\Model\AddressCreate() $address address.
	...
	protected function set_address_dob( $address, $date_of_birth_string ) {
		if ( ! empty( $date_of_birth_string ) ) {
			$date_of_birth = WC_VRPayment_Helper::instance()->try_to_parse_date( $date_of_birth_string );
	...
	protected function set_address_gender( $address, $gender_string ) {
		if ( ! empty( $gender_string ) ) {
			if ( strtolower( $gender_string ) == 'm' || strtolower( $gender_string ) == 'male' ) {
				$address->setGender( \VRPayment\Sdk\Model\Gender::MALE );
			} elseif ( strtolower( $gender_string ) == 'f' || strtolower( $gender_string ) == 'female' ) {
				$address->setGender( \VRPayment\Sdk\Model\Gender::FEMALE );
			}
		}
	}
	 ...
	 * @return void
	 */
	protected function set_modified_order_line_items( WC_Order $order, $order_total, \VRPayment\Sdk\Model\AbstractTransactionPending $transaction ) {
		$transaction->setLineItems( WC_VRPayment_Service_Line_Item::instance()->get_items_for_renewal( $order, $order_total, $transaction ) );
	}
	...
	public function create_transaction_by_renewal_order( WC_Order $order, $order_total, $token_id ) {
		...
		$create_transaction = apply_filters( 'wc_vrpayment_subscription_create_transaction', $create_transaction, $order );
		if (!$create_transaction->valid()) {
			throw new Exception("The transaction you are trying to create is not valid.");
		}
		WC_VRPayment_Helper::instance()->add_headers( $this->api_client, [
			WC_VRPayment_Helper::SUBSCRIPTION_TRANSACTION => true
		]);
		$transaction = $this->get_transaction_service()->create( $space_id, $create_transaction );
		$this->update_transaction_info( $transaction, $order );
		return $transaction;
	 ...
	 * @throws Exception Exception.
	 */
	public function update_transaction_by_renewal_order( WC_Order $order, $order_total, $token_id, \VRPayment\Sdk\Model\Transaction $transaction ) {
		$last = new \VRPayment\Sdk\VersioningException();
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				$pending_transaction = new \VRPayment\Sdk\Model\TransactionPending();
				$pending_transaction->setId( $transaction->getId() );
				...
				$pending_transaction = apply_filters( 'wc_vrpayment_subscription_update_transaction', $pending_transaction, $order );
				WC_VRPayment_Helper::instance()->add_headers( $this->api_client, [
					WC_VRPayment_Helper::SUBSCRIPTION_TRANSACTION => true
				]);
	...
	public function store_performance_metric( $metric, $timestamp, $transaction_id ) {
		...
		$metrics = $session->get( 'vrpayment_performance_metrics', array() );
		$metrics[] = array(
			'metric' => $metric,
			'timestamp' => $timestamp,
			'transaction_id' => $transaction_id,
		);
		$session->set( 'vrpayment_performance_metrics', $metrics );
	}

	/**
	 * Adds performance metrics headers to the API client.
	 *
	 * @param \VRPayment\Sdk\ApiClient $api_client The API client.
	 */
	private function add_performance_metrics_headers( \VRPayment\Sdk\ApiClient $api_client ) {
		$session = WC()->session;
		if ( ! $session ) {
			return;
		}
