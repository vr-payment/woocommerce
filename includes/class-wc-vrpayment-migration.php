...
class WC_VRPayment_Migration {
	...
	private static function check_requirements() {
		...
		if ( ! empty( $errors ) ) {
			...
			$message = printf(
				...
				esc_html__( 'Please check the following requirements before activating:', 'woo-vrpayment' ),
				esc_html__( $error_list ) //phpcs:ignore
			);
			// phpcs:ignore
			wp_die( $message, esc_html__( 'Could not activate plugin VRPay.', 'woo-vrpayment' ), array( 'back_link' => true ) );
			return;
		}
	}
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( WC_VRPAYMENT_PLUGIN_BASENAME === $file ) {
			$row_meta = array(
				'docs' => '<a href="https://gateway.vr-payment.de/doc/woocommerce/3.3.3/docs/en/documentation.html" aria-label="' . esc_html__( 'View Documentation', 'woo-vrpayment' ) . '">' . esc_html__( 'Documentation', 'woo-vrpayment' ) . '</a>',
			);
