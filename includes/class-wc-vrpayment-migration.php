...
class WC_VRPayment_Migration {
	...
	const VRPAYMENT_CK_DB_VERSION = 'wc_vrpayment_db_version';
	 ...
	 */
	const VRPAYMENT_DEPRECATED_TABLE_PREFIX = 'wc_';
	const VRPAYMENT_DEPRECATED_PLUGIN_PREFIX = 'woo-';
	...
	public static function install_db( $networkwide ) {
		...
		wc_maybe_define_constant( 'WC_VRPAYMENT_INSTALLING', true );
	...
	private static function check_requirements() {
		...
		if ( version_compare( PHP_VERSION, WC_VRPAYMENT_REQUIRED_PHP_VERSION, '<' ) ) {
			$errors[] = sprintf( esc_html__( "PHP %1\$s+ is required. (You're running version %2\$s)", 'woo-vrpayment' ), WC_VRPAYMENT_REQUIRED_PHP_VERSION, PHP_VERSION );
		}
		if ( version_compare( $wp_version, WC_VRPAYMENT_REQUIRED_WP_VERSION, '<' ) ) {
			$errors[] = sprintf( esc_html__( "WordPress %1\$s+ is required. (You're running version %2\$s)", 'woo-vrpayment' ), WC_VRPAYMENT_REQUIRED_WP_VERSION, $wp_version );

		}

		if ( ! is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			/* translators: %s+: version */
			$errors[] = sprintf( esc_html__( 'Woocommerce %s+ has to be active.', 'woo-vrpayment' ), WC_VRPAYMENT_REQUIRED_WC_VERSION );
		} else {
			$woocommerce_data = get_plugin_data( WP_PLUGIN_DIR . '/woocommerce/woocommerce.php', false, false );

			if ( version_compare( $woocommerce_data['Version'], WC_VRPAYMENT_REQUIRED_WC_VERSION, '<' ) ) {
				$errors[] = sprintf( esc_html__( "Woocommerce %1\$s+ is required. (You're running version %2\$s)", 'woo-vrpayment' ), WC_VRPAYMENT_REQUIRED_WC_VERSION, $woocommerce_data['Version'] );
			}
		}
		...
		try {
			...
			$errors[] = __( "Install the PHP cUrl extension or ensure the 'stream_socket_client' function is available.", 'woo-vrpayment' );
		}
		...
		if ( ! empty( $errors ) ) {
			...
			wp_die(
				...
				esc_html__( 'Could not activate plugin VR Payment.', 'woo-vrpayment' ),
	...
	public static function wp_initialize_site( WP_Site $site, array $args ) { //phpcs:ignore
		if ( is_plugin_active_for_network( 'woo-vrpayment/woocommerce-vrpayment.php' ) ) {
			$blog_id = $site->blog_id;
			// Defensive check: blog_id should always be set, otherwise will be shown as vrpayment error
			if ( ! $blog_id ) {
				$logger = wc_get_logger();
				$logger->error( 'Could not detect blog_id during wp_initialize_site.', [ 'source' => 'vrpayment' ] );
				return;
			}
	...
	private static function migrate_db() {
		$current_version = get_option( self::VRPAYMENT_CK_DB_VERSION, 0 );
		foreach ( self::$db_migrations as $version => $function_name ) {
			if ( version_compare( $current_version, $version, '<' ) ) {
				...
				update_option( self::VRPAYMENT_CK_DB_VERSION, $version );
	...
	public static function wpmu_drop_tables( $tables ) {
		...
		$tables[] = $wpdb->prefix . 'vrpayment_method_config';
		$tables[] = $wpdb->prefix . 'vrpayment_transaction_info';
		$tables[] = $wpdb->prefix . 'vrpayment_token_info';
		$tables[] = $wpdb->prefix . 'vrpayment_completion_job';
		$tables[] = $wpdb->prefix . 'vrpayment_void_job';
		$tables[] = $wpdb->prefix . 'vrpayment_refund_job';

		return $tables;
	}

	/**
	 * Check VRPayment DB version and run the migration if required.
	 *
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( WC_VRPAYMENT_PLUGIN_BASENAME === $file ) {
			$row_meta = array(
				'docs' => '<a href="https://docs.plugin-documentation.vr-payment.de/vr-payment/woocommerce/3.3.19/docs/en/documentation.html" aria-label="' . esc_html__( 'View Documentation', 'woo-vrpayment' ) . '">' . esc_html__( 'Documentation', 'woo-vrpayment' ) . '</a>',
			);
	...
	public static function update_1_0_0_initialize() {
		...
		$table_method_config = $wpdb->prefix . 'vrpayment_method_config';
		...
		$table_transaction_info = $wpdb->prefix . 'vrpayment_transaction_info';
		...
		$table_token_info = $wpdb->prefix . 'vrpayment_token_info';
		...
		$table_completion_job = $wpdb->prefix . 'vrpayment_completion_job';
		...
		$table_void_job = $wpdb->prefix . 'vrpayment_void_job';
		...
		$table_refund_job = $wpdb->prefix . 'vrpayment_refund_job';
	...
	public static function update_1_0_3_image_domain() {
		...
		$table_transaction_info = $wpdb->prefix . 'vrpayment_transaction_info'; //phpcs:ignore
		$table_method_config = $wpdb->prefix . 'vrpayment_method_config'; //phpcs:ignore
	...
	public static function update_1_0_4_failure_msg_and_attribute() {
		...
		$table_transaction_info = $wpdb->prefix . 'vrpayment_transaction_info'; //phpcs:ignore
		$table_attribute_options = $wpdb->prefix . 'vrpayment_attribute_options'; //phpcs:ignore
	...
	public static function update_1_0_6_shorten_table_names() {
		...
		$table_prefix = self::VRPAYMENT_DEPRECATED_TABLE_PREFIX;
		$table_rename_map = array(
			"{$wpdb->prefix}{$table_prefix}woocommerce_vrpayment_attribute_options" => "{$wpdb->prefix}{$table_prefix}vrpayment_attribute_options",
			"{$wpdb->prefix}{$table_prefix}woocommerce_vrpayment_completion_job" => "{$wpdb->prefix}{$table_prefix}vrpayment_completion_job",
			"{$wpdb->prefix}{$table_prefix}woocommerce_vrpayment_method_configuration" => "{$wpdb->prefix}{$table_prefix}vrpayment_method_config",
			"{$wpdb->prefix}{$table_prefix}woocommerce_vrpayment_refund_job" => "{$wpdb->prefix}{$table_prefix}vrpayment_refund_job",
			"{$wpdb->prefix}{$table_prefix}woocommerce_vrpayment_token_info" => "{$wpdb->prefix}{$table_prefix}vrpayment_token_info",
			"{$wpdb->prefix}{$table_prefix}woocommerce_vrpayment_transaction_info" => "{$wpdb->prefix}{$table_prefix}vrpayment_transaction_info",
			"{$wpdb->prefix}{$table_prefix}woocommerce_vrpayment_void_job" => "{$wpdb->prefix}{$table_prefix}vrpayment_void_job",
		);
	...
	public static function update_1_0_7_migrate_plugin_name_and_tables() {
		...
		$table_prefix = self::VRPAYMENT_DEPRECATED_TABLE_PREFIX;
		$table_rename_map = array(
			"{$wpdb->prefix}{$table_prefix}vrpayment_attribute_options" => "{$wpdb->prefix}vrpayment_attribute_options",
			"{$wpdb->prefix}{$table_prefix}vrpayment_completion_job" => "{$wpdb->prefix}vrpayment_completion_job",
			"{$wpdb->prefix}{$table_prefix}vrpayment_method_config" => "{$wpdb->prefix}vrpayment_method_config",
			"{$wpdb->prefix}{$table_prefix}vrpayment_refund_job" => "{$wpdb->prefix}vrpayment_refund_job",
			"{$wpdb->prefix}{$table_prefix}vrpayment_token_info" => "{$wpdb->prefix}vrpayment_token_info",
			"{$wpdb->prefix}{$table_prefix}vrpayment_transaction_info" => "{$wpdb->prefix}vrpayment_transaction_info",
			"{$wpdb->prefix}{$table_prefix}vrpayment_void_job" => "{$wpdb->prefix}vrpayment_void_job",
		);
		self::rename_tables( $table_rename_map );
	}
	...
	public static function supported_payments_integration_notice(): void {
			...
			add_action(
				...
				function () {
					...
					<div class="notice notice-error">
						<p><?php __( 'WooCommerce is not activated. Please activate WooCommerce to use the payment integration.', 'woo-vrpayment' ); ?></p>
					</div>
					<?php
				}
			);
		...
		$woocommerce_data = get_plugin_data( WP_PLUGIN_DIR . '/woocommerce/woocommerce.php', false, false );
		if ( version_compare( $woocommerce_data['Version'], WC_VRPAYMENT_REQUIRED_WC_MAXIMUM_VERSION, '>' ) ) {
			$notice_id = "vrpayment-{$woocommerce_data['Version']}-not-yet-supported";
			if ( ! WC_Admin_Notices::user_has_dismissed_notice( $notice_id ) ) {
				$message = sprintf(
					...
					__( 'The plugin VRPayment has been tested up to WooCommerce %1$s but you have installed the version %2$s. Please notice that this is not recommended.', 'woo-vrpayment' ),
					WC_VRPAYMENT_REQUIRED_WC_MAXIMUM_VERSION,
				...
				$previous_version = get_user_meta( get_current_user_id(), 'vrpayment-previous-wc-min-version' );
				if ( $previous_version ) {
					delete_user_meta( get_current_user_id(), "dismissed_vrpayment-{$previous_version[0]}-not-yet-supported_notice" );
				}
				update_user_meta( get_current_user_id(), 'vrpayment-previous-wc-min-version', $woocommerce_data['Version'] );
			}
		}
	}
	...
	public static function update_1_0_8_store_default_status_mappings() {
		$status_adapter = new WC_VRPayment_Order_Status_Adapter();
