...
/**
 * WC VRPayment Admin Order Completion class
 */
class WC_VRPayment_Admin_Order_Completion {
	...
	public static function init() {
		...
		add_action(
			'wp_ajax_woocommerce_vrpayment_execute_completion',
			array(
				__CLASS__,
				'execute_completion',
			)
	...
	public static function render_execute_completion_button( WC_Order $order ) {
		...
		if ( $gateway instanceof WC_VRPayment_Gateway ) {
			...
			if ( TransactionState::AUTHORIZED === $transaction_info->get_state() ) {
				echo '<button type="button" class="button vrpayment-completion-button action-vrpayment-completion-cancel" style="display:none">' .
					esc_html__( 'Cancel', 'woo-vrpayment' ) . '</button>';
				echo '<button type="button" class="button button-primary vrpayment-completion-button action-vrpayment-completion-execute" style="display:none">' .
					esc_html__( 'Execute Completion', 'woo-vrpayment' ) . '</button>';
				echo '<label for="completion_restock_not_completed_items" style="display:none">' .
					esc_html__( 'Restock not completed items', 'woo-vrpayment' ) . '</label>';
				...
				echo '<label for="refund_amount" style="display:none">' . esc_html__( 'Completion Amount', 'woo-vrpayment' ) . '</label>';
			}
		}
	}
	...
	public static function execute_completion() {
		...
		try {
				...
				if ( isset( $item['completion_tax'] ) && is_array( $item['completion_tax'] ) ) {
					foreach ( $item['completion_tax'] as $rate_id => $amount ) {
						...
						if ( wc_format_decimal( $tax_amount, wc_get_price_decimals() ) !== wc_format_decimal( $amount, wc_get_price_decimals() ) ) {
							throw new Exception( __( 'The tax rate can not be changed.', 'woo-vrpayment' ) );
						}
					}
			...
			if ( wc_format_decimal( $completion_amount, wc_get_price_decimals() ) !== wc_format_decimal( $total_items_sum, wc_get_price_decimals() ) ) {
				throw new Exception( __( 'The line item total does not correspond to the total amount to complete.', 'woo-vrpayment' ) );
			}
			...
			if ( $transaction_info->get_state() !== TransactionState::AUTHORIZED ) {
				throw new Exception( __( 'The transaction is not in a state to be completed.', 'woo-vrpayment' ) );
			}
			...
			if ( WC_VRPayment_Entity_Completion_Job::count_running_completion_for_transaction(
				...
				throw new Exception( __( 'Please wait until the existing completion is processed.', 'woo-vrpayment' ) );
			}
			if ( WC_VRPayment_Entity_Void_Job::count_running_void_for_transaction(
				$transaction_info->get_space_id(),
				$transaction_info->get_transaction_id()
			) > 0 ) {
				throw new Exception( __( 'There is a void in process. The order can not be completed.', 'woo-vrpayment' ) );
			}

			$completion_job = new WC_VRPayment_Entity_Completion_Job();
			...
			$completion_job->set_state( WC_VRPayment_Entity_Completion_Job::VRPAYMENT_STATE_CREATED );
		...
		try {
			...
			WC()->session->set( 'vrpayment_order_id', $order_id );
			self::update_line_items( $current_completion_id );
			self::send_completion( $current_completion_id );

			wp_send_json_success(
				array(
					'message' => __( 'The completion is updated automatically once the result is available.', 'woo-vrpayment' ),
				)
			);
		} catch ( Exception $e ) {
			wp_send_json_error(
	 ...
	 * @return void
	 * @throws \VRPayment\Sdk\ApiException Api exception.
	 * @throws \VRPayment\Sdk\Model\ClientError Client Error.
	 */
	protected static function update_line_items( $completion_job_id ) {
		$completion_job = WC_VRPayment_Entity_Completion_Job::load_by_id( $completion_job_id );
		WC_VRPayment_Helper::instance()->start_database_transaction();
		WC_VRPayment_Helper::instance()->lock_by_transaction_id( $completion_job->get_space_id(), $completion_job->get_transaction_id() );
		// Reload void job.
		$completion_job = WC_VRPayment_Entity_Completion_Job::load_by_id( $completion_job_id );

		if ( $completion_job->get_state() !== WC_VRPayment_Entity_Completion_Job::VRPAYMENT_STATE_CREATED ) {
			// Already updated in the meantime.
			WC_VRPayment_Helper::instance()->rollback_database_transaction();
			return;
		}
		try {
			$line_items = WC_VRPayment_Service_Line_Item::instance()->get_items_from_backend(
				$completion_job->get_items(),
				$completion_job->get_amount(),
				WC_Order_Factory::get_order( $completion_job->get_order_id() )
			);
			WC_VRPayment_Service_Transaction::instance()->update_line_items(
				$completion_job->get_space_id(),
				$completion_job->get_transaction_id(),
				$line_items
			);
			$completion_job->set_state( WC_VRPayment_Entity_Completion_Job::VRPAYMENT_STATE_ITEMS_UPDATED );
			$completion_job->save();
			WC_VRPayment_Helper::instance()->commit_database_transaction();
		} catch ( \VRPayment\Sdk\ApiException $e ) {
			if ( $e->getResponseObject() instanceof \VRPayment\Sdk\Model\ClientError ) {
				$completion_job->set_state( WC_VRPayment_Entity_Completion_Job::VRPAYMENT_STATE_DONE );
				$completion_job->save();
				WC_VRPayment_Helper::instance()->commit_database_transaction();
			} else {
				$completion_job->save();
				WC_VRPayment_Helper::instance()->commit_database_transaction();
				WooCommerce_VRPayment::instance()->log( 'Error updating line items. ' . $e->getMessage(), WC_Log_Levels::INFO );
				throw $e;
			}
		} catch ( Exception $e ) {
			$completion_job->save();
	 ...
	 * @return void
	 *
	 * @throws \VRPayment\Sdk\ApiException ClientError.
	 * @throws Exception Exception.
	 */
	protected static function send_completion( $completion_job_id ) {
		...
		if ( $completion_job->get_state() !== WC_VRPayment_Entity_Completion_Job::VRPAYMENT_STATE_ITEMS_UPDATED ) {
			// Already sent in the meantime.
			WC_VRPayment_Helper::instance()->rollback_database_transaction();
			return;
		}
		try {
			$completion_service = new \VRPayment\Sdk\Service\TransactionCompletionService( WC_VRPayment_Helper::instance()->get_api_client() );
			...
			$completion_job->set_state( WC_VRPayment_Entity_Completion_Job::VRPAYMENT_STATE_SENT );
			$completion_job->save();
			WC_VRPayment_Helper::instance()->commit_database_transaction();
		} catch ( \VRPayment\Sdk\ApiException $e ) {
			if ( $e->getResponseObject() instanceof \VRPayment\Sdk\Model\ClientError ) {
				...
				$completion_job->save();
				WC_VRPayment_Helper::instance()->commit_database_transaction();
				WooCommerce_VRPayment::instance()->log( 'Error sending completion. ' . $e->getMessage(), WC_Log_Levels::INFO );
				throw $e;
			}
		} catch ( Exception $e ) {
			$completion_job->save();
	...
	public static function update_for_order( WC_Order $order ) {
		...
		$transaction_info = WC_VRPayment_Entity_Transaction_Info::load_by_order_id( $order->get_id() );
		$completion_job   = WC_VRPayment_Entity_Completion_Job::load_running_completion_for_transaction( $transaction_info->get_space_id(), $transaction_info->get_transaction_id() );

		if ( $completion_job->get_state() === WC_VRPayment_Entity_Completion_Job::VRPAYMENT_STATE_CREATED ) {
			self::update_line_items( $completion_job->get_id() );
			self::send_completion( $completion_job->get_id() );
		} elseif ( $completion_job->get_state() === WC_VRPayment_Entity_Completion_Job::VRPAYMENT_STATE_ITEMS_UPDATED ) {
			self::send_completion( $completion_job->get_id() );
		}
	}
	...
	public static function update_completions() {
		$to_process = WC_VRPayment_Entity_Completion_Job::load_not_sent_job_ids();
		foreach ( $to_process as $id ) {
			try {
				...
				$message = sprintf( __( 'Error updating completion job with id %1$d: %2$s', 'woo-vrpayment' ), $id, $e->getMessage() );
				WooCommerce_VRPayment::instance()->log( $message, WC_Log_Levels::ERROR );
			}
		}
	}
}
WC_VRPayment_Admin_Order_Completion::init();
