...
class WC_VRPayment_Zero_Gateway extends WC_Payment_Gateway {

	public const ZERO_PAYMENT_CONF_ID = 999999;

	public function __construct() {
		$this->id = 'vrpayment_zero';
		$this->method_title = __( 'No Payment Required', 'woo-vrpayment' );
		$this->method_description = __( 'Used when order total is zero.', 'woo-vrpayment' );
		$this->title = __( 'No Payment Required', 'woo-vrpayment' );
		$this->enabled = 'yes';
	...
	public function init_form_fields() {
		$this->form_fields = array(
		  'enabled' => array(
			'title'   => __( 'Enable/Disable', 'woo-vrpayment' ),
			'type'    => 'checkbox',
			'label'   => __( 'Enable No Payment Required Gateway', 'woo-vrpayment' ),
			'default' => 'yes',
		  ),
		  'title' => array(
			'title'       => __( 'Title', 'woo-vrpayment' ),
			'type'        => 'text',
			'description' => __( 'This controls the title seen by the customer during checkout.', 'woo-vrpayment' ),
			'default'     => __( 'No Payment Required', 'woo-vrpayment' ),
			'desc_tip'    => true,
		  ),
		);
	}
	...
	public function process_payment( $order_id ) {
		...
		$order->add_order_note( __( 'Order completed automatically – no payment needed.', 'woo-vrpayment' ) );
