...
/**
 * Class WC_VRPayment_Order_Status_Adapter.
 *
 * This class handles the mapping and updating of order statuses in WooCommerce
 * based on the transaction statuses in VRPayment.
 */
class WC_VRPayment_Order_Status_Adapter
{
	/**
	 * Constants for VRPayment transaction statuses.
	 */
	const VRPAYMENT_STATUS_PENDING = 'pending';
	const VRPAYMENT_STATUS_CONFIRMED = 'confirmed';
	const VRPAYMENT_STATUS_PROCESSING = 'processing';
	const VRPAYMENT_STATUS_AUTHORIZED = 'authorized';
	const VRPAYMENT_STATUS_COMPLETED = 'completed';
	const VRPAYMENT_STATUS_FAILED = 'failed';
	const VRPAYMENT_STATUS_VOIDED = 'voided';
	const VRPAYMENT_STATUS_FULFILL = 'fulfill';
	const VRPAYMENT_STATUS_DECLINE = 'decline';

	const VRPAYMENT_ORDER_STATUS_MAPPING_PREFIX = 'vrpayment_order_status_mapping_';
	const VRPAYMENT_CUSTOM_ORDER_STATUS_PREFIX = 'vrpayment_custom_order_status_';
	/**
	...
	/**
	 * WC_VRPayment_Order_Status_Adapter constructor.
	 *
	...
	{
		add_action( 'plugins_loaded', array( __CLASS__, 'register_vrpayment_service_status_adapter' ) );
	}

	/**
	 * Registers the VRPayment service status adapter.
	 *
	 * This function initializes an instance of the WC_VRPayment_Order_Status_Adapter class.
	 * It is hooked to the 'plugins_loaded' action to ensure it runs after all plugins are loaded.
	 *
	 * @return void
	 */
	public static function register_vrpayment_service_status_adapter(): void
	{
		new WC_VRPayment_Order_Status_Adapter();
	}
	...
	{
		add_filter( 'vrpayment_default_order_status_mappings', array( $this, 'get_default_status_mappings' ) );
		add_filter( 'vrpayment_woocommerce_statuses', array( $this, 'get_all_woocommerce_statuses' ) );
		add_filter( 'vrpayment_order_statuses', array( $this, 'get_vrpayment_statuses' ) );
		add_filter( 'vrpayment_order_update_status', array( $this, 'update_order_status' ), 10, 5 );
		add_filter( 'vrpayment_wc_status_for_transaction', array( $this, 'get_wc_status_for_transaction' ), 10, 1 );
	...
	public function add_order_statuses( $order_statuses ) {
		...
		$prefix = self::VRPAYMENT_CUSTOM_ORDER_STATUS_PREFIX . '%'; //We use % as wildcard for LIKE.
		...
		foreach ( $results as $row ) {
			...
			$order_statuses[ $row['option_value'] ] = sprintf(
				...
				_x( '%s', 'Order status label', 'woo-vrpayment' ),
				$status_label
			);
		}
	...
	public function valid_order_statuses_for_payment( $order_statuses ) {
		$default_mappings = array(
			self::VRPAYMENT_STATUS_PENDING => 'wc-pending',
			self::VRPAYMENT_STATUS_CONFIRMED => 'wc-on-hold',
			self::VRPAYMENT_STATUS_PROCESSING => 'wc-on-hold',
			self::VRPAYMENT_STATUS_COMPLETED => 'wc-processing',
			self::VRPAYMENT_STATUS_FULFILL => 'wc-completed',
		);
	...
	{
		$is_custom_mapping_enabled = WC_VRPayment_Helper::is_custom_status_mapping_enabled();
		...
		foreach ( $default_mappings as $transaction_status => $default_order_status ) {
			$option_key = self::VRPAYMENT_ORDER_STATUS_MAPPING_PREFIX . $transaction_status;
	...
	{
		return strpos( $order_status, 'wc-vrpaym-' ) === 0 || strpos( $order_status, 'vrpaym-' ) === 0;
	}
	 ...
	 * `vrpayment_order_update_status_<status>`.
	 *
	 * @since 1.0.0
	...
	public function store_default_status_mappings_on_database(): void
	{
		...
		foreach ( $default_mappings as $key => $value ) {
			$result = update_option( self::VRPAYMENT_ORDER_STATUS_MAPPING_PREFIX . $key, $value );
		}
	}
	 ...
	 *
	 * This method defines the default mappings between VRPayment transaction
	 * statuses and WooCommerce order statuses. These mappings are saved in the WordPress
	 * `wp_options` table as `vrpayment_order_status_mapping_<status>` when the
	 ...
	 * - vrpayment_order_status_mapping_pending -> wc-pending -> OrderInternalStatus::PENDING
	 * - vrpayment_order_status_mapping_confirmed -> wc-on-hold -> OrderInternalStatus::ON_HOLD
	 * - vrpayment_order_status_mapping_processing -> wc-on-hold -> OrderInternalStatus::ON_HOLD
	 * - vrpayment_order_status_mapping_authorized -> wc-on-hold -> OrderInternalStatus::ON_HOLD
	 * - vrpayment_order_status_mapping_completed -> wc-vrpaym-waiting -> OrderInternalStatus::ON_HOLD
	 * - vrpayment_order_status_mapping_failed -> wc-failed -> OrderInternalStatus::FAILED
	 * - vrpayment_order_status_mapping_voided -> wc-cancelled or 'wc-refunded' -> OrderInternalStatus::CANCELLED
	 * - vrpayment_order_status_mapping_fulfill -> wc-processing -> OrderInternalStatus::PROCESSING
	 *
	 * Example when relying on WooCommerce core statuses:
	 * - vrpayment_order_status_mapping_processing -> wc-processing -> OrderInternalStatus::PROCESSING
	 * - vrpayment_order_status_mapping_completed -> wc-processing -> OrderInternalStatus::PROCESSING
	 *
	...
	public function get_default_status_mappings() : array
	{
		$default_mappings = array(
			...
			self::VRPAYMENT_STATUS_CONFIRMED => 'wc-on-hold',
			self::VRPAYMENT_STATUS_PROCESSING => 'wc-on-hold',
			self::VRPAYMENT_STATUS_AUTHORIZED => 'wc-on-hold',
			self::VRPAYMENT_STATUS_COMPLETED => 'wc-on-hold',
			self::VRPAYMENT_STATUS_FAILED => 'wc-failed',
			self::VRPAYMENT_STATUS_VOIDED => 'wc-cancelled',
			self::VRPAYMENT_STATUS_FULFILL => 'wc-processing',
			self::VRPAYMENT_STATUS_DECLINE => 'wc-cancelled',
		);

		if ( WC_VRPayment_Helper::is_custom_status_mapping_enabled() ) {
			$default_mappings[ self::VRPAYMENT_STATUS_COMPLETED ] = 'wc-vrpaym-waiting';
		}
	 ...
	 * - vrpayment_order_status_mapping_confirmed -> vrpaym-redirected
	 * - vrpayment_order_status_mapping_processing -> vrpaym-redirected
	 * - vrpayment_order_status_mapping_completed -> vrpaym-waiting
	 *
	 * These defaults are used if no custom mappings are provided by the user.
	 *
	 * @return array
	...
	{
		return array(
			self::VRPAYMENT_STATUS_CONFIRMED => 'vrpaym-redirected',
			self::VRPAYMENT_STATUS_PROCESSING => 'vrpaym-redirected',
			self::VRPAYMENT_STATUS_COMPLETED => 'vrpaym-waiting',
		);
	}
	...
	/**
	 * Gets the VRPayment statuses.
	 *
	 * @return array
	 */
	public function get_vrpayment_statuses(): array
	{
		return array(
			self::VRPAYMENT_STATUS_PENDING => ucwords( self::VRPAYMENT_STATUS_PENDING ),
			self::VRPAYMENT_STATUS_CONFIRMED => ucwords( self::VRPAYMENT_STATUS_CONFIRMED ),
			self::VRPAYMENT_STATUS_PROCESSING => ucwords( self::VRPAYMENT_STATUS_PROCESSING ),
			self::VRPAYMENT_STATUS_AUTHORIZED => ucwords( self::VRPAYMENT_STATUS_AUTHORIZED ),
			self::VRPAYMENT_STATUS_COMPLETED => ucwords( self::VRPAYMENT_STATUS_COMPLETED ),
			self::VRPAYMENT_STATUS_FAILED => ucwords( self::VRPAYMENT_STATUS_FAILED ),
			self::VRPAYMENT_STATUS_VOIDED => ucwords( self::VRPAYMENT_STATUS_VOIDED ),
			self::VRPAYMENT_STATUS_FULFILL => ucwords( self::VRPAYMENT_STATUS_FULFILL ),
			self::VRPAYMENT_STATUS_DECLINE => ucwords( self::VRPAYMENT_STATUS_DECLINE ),
		);
	}

	/**
	 * Gets the WooCommerce status corresponding to a VRPayment status.
	 *
	 * @param string|null $status The VRPayment transaction status.
	 * @return string|null The corresponding WooCommerce order status or null if not found.
	 */
	private function map_vrpayment_status_to_woocommerce( ?string $status ): ?string
	{
	...
	/**
	 * Updates the status of a WooCommerce order based on the VRPayment status.
	 *
	 * @param WC_Order $order $order The WooCommerce order.
	 * @param string|null $status The VRPayment transaction status.
	 * @param string|null $default status The VRPayment transaction status by default.
	...
	public function update_order_status( WC_Order $order, ?string $status, ?string $default_status, string $note = '', bool $manual = false ): void
	{
		...
		$status = apply_filters( 'vrpayment_pre_order_update_status', $status, $order, $note, $manual );
		$change_setting_enabled = 'yes' === get_option( WooCommerce_VRPayment::VRPAYMENT_CK_CHANGE_ORDER_STATUS );
		$is_virtual_order = WC_VRPayment_Helper::is_order_virtual( $order );
		$should_force_completed = (
			$change_setting_enabled
			&& $is_virtual_order
			&& null !== $status
			&& strtolower( (string) $status ) === strtolower( \VRPayment\Sdk\Model\TransactionState::FULFILL )
		);

		// Preserve WooCommerce's completed status only once the transaction reaches FULFILL.
		$new_status = $should_force_completed ? 'completed' : $this->map_vrpayment_status_to_woocommerce( $status );
		...
		if ( !empty( $new_status ) && !empty( $order ) ) {
			...
			apply_filters( 'vrpayment_post_order_update_status', $status, $order, $note, $manual );
		}
	}
	...
	public function get_order_status_on_payment_complete( string $status, int $order_id, WC_Order $order ): string
	{
		...
		$is_fulfilled = $transaction_info
			&& strtolower( $transaction_info->get_state() ) === strtolower( \VRPayment\Sdk\Model\TransactionState::FULFILL );
		...
		if ( $change_setting_enabled && $is_virtual_order && $is_fulfilled ) {
			return self::VRPAYMENT_STATUS_COMPLETED;
		}
		
		// Check if the transaction status is mapped in WooCommerce
		$mapped_status = $this->map_vrpayment_status_to_woocommerce( \VRPayment\Sdk\Model\TransactionState::FULFILL );
	...
	/**
	 * Retrieves the mapped WooCommerce order status for a given VRPayment transaction status.
	 *
	 * This method fetches the corresponding WooCommerce order status that has been mapped
	 * to a transaction status from the VRPayment portal. The mapping is stored
	 * in the `wp_options` table using the naming pattern:
	 * `vrpayment_order_status_mapping_<transaction_status>`.
	 *
	 * Expected values for `$vrpayment_status`: Pending, Confirmed, Processing, Failed, Authorized, Voided, Completed, Fulfill, Decline.
	 *
	 * @since 1.0.0
	 * @param string|null $vrpayment_status The transaction status from VRPayment.
	 * @return string|null The mapped WooCommerce order status, or null if no mapping is found.
	 */
	public function get_wc_status_for_transaction( ?string $vrpayment_status ): ?string
	{
		if ( null === $vrpayment_status || '' === $vrpayment_status ) {
			return null;
		}

		$status = get_option( self::VRPAYMENT_ORDER_STATUS_MAPPING_PREFIX . strtolower( $vrpayment_status ), null );
		if ( ! WC_VRPayment_Helper::is_custom_status_mapping_enabled() && $status && $this->is_custom_order_status( $status ) ) {
			$default_mappings = $this->get_default_status_mappings();
			$status_key = strtolower( $vrpayment_status );
...
WC_VRPayment_Order_Status_Adapter::init();
