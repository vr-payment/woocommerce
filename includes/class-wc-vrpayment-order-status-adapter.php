...
class WC_VRPayment_Order_Status_Adapter
{
	...
	private function initialize_status_mappings(): void
	{
		...
		$this->settings = array_map( function ( $transaction_status, $default_order_status ) {
			return array(
				...
				'order_status' => get_option( self::VRPAYMENT_ORDER_STATUS_MAPPING_PREFIX . $transaction_status, $default_order_status )
			);
	 ...
	 * - vrpayment_order_status_mapping_completed -> wc-on-hold -> OrderInternalStatus::ON_HOLD
	...
	public function get_default_status_mappings() : array
	{
		return array(
			...
			self::VRPAYMENT_STATUS_COMPLETED => 'wc-vrpaym-waiting',
	 ...
	 *
	 * @param string $status The VRPayment transaction status.
	...
	public function update_order_status( WC_Order $order, ?string $status, ?string $default_status, string $note = '', bool $manual = false ): void
	{
		...
		if ( $status === null && $order !== null ) {
			$transaction_info = VRPayment_Entity_Transaction_Info::load_by_order_id( $order->get_id() );
			if ( $transaction_info ) {
		...
		$new_status = $this->map_vrpayment_status_to_woocommerce( $status );
	...
	public function get_order_status_on_payment_complete( string $status, int $order_id, WC_Order $order ): string
	{
		...
		if ( 'yes' === get_option( WooCommerce_VRPayment::VRPAYMENT_CK_CHANGE_ORDER_STATUS ) 
		&& $order->get_total() <= 0 && WC_VRPayment_Helper::is_order_virtual( $order ) ) {
	 ...
	 *
	 * @since 1.0.0
	 * @param string $vrpayment_status The transaction status from VRPayment.
