...
/**
 * Class WC_VRPayment_Entity_Abstract.
 * Abstract implementation of a entity
 *
 * @class WC_VRPayment_Entity_Abstract
 */
abstract class WC_VRPayment_Entity_Abstract {
	/**
	 * Data
	 *
	 * @var $data data.
	...
	protected static function get_base_fields() {
		return array(
			...
			'created_at' => WC_VRPayment_Entity_Resource_Type::VRPAYMENT_DATETIME,
			'updated_at' => WC_VRPayment_Entity_Resource_Type::VRPAYMENT_DATETIME,
		);
	}
	...
	protected function fill_values_from_db( array $db_values ) {
		...
		foreach ( $fields as $key => $type ) {
			if ( isset( $db_values[ $key ] ) ) {
				$value = $db_values[ $key ];
				switch ( $type ) {
					case WC_VRPayment_Entity_Resource_Type::VRPAYMENT_STRING:
						// Do nothing.
						break;
					case WC_VRPayment_Entity_Resource_Type::VRPAYMENT_BOOLEAN:
						$value = 'Y' === $value;
						break;
					case WC_VRPayment_Entity_Resource_Type::VRPAYMENT_INTEGER:
						$value = intval( $value );
						break;

					case WC_VRPayment_Entity_Resource_Type::VRPAYMENT_DECIMAL:
						$value = (float) $value;
						break;

					case WC_VRPayment_Entity_Resource_Type::VRPAYMENT_DATETIME:
						$value = new DateTime( $value );
						break;

					case WC_VRPayment_Entity_Resource_Type::VRPAYMENT_OBJECT:
