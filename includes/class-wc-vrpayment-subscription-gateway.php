...
/**
 * This class implements the VR Payment subscription gateways
 */
class WC_VRPayment_Subscription_Gateway {

	/**
	 * Gateway.
	 *
	 * @var WC_VRPayment_Gateway $gateway Gateway.
	 */
	private $gateway;
	 ...
	 */
	public function __construct( WC_VRPayment_Gateway $gateway ) {
	...
	public function process_scheduled_subscription_payment( $amount_to_charge, WC_Order $order ) {
		try {
			...
			$token_space_id = $token_data['_vrpayment_subscription_space_id'];
			$token_id = $token_data['_vrpayment_subscription_token_id'];
			...
			if ( $transaction_info->get_id() > 0 ) {
				...
				if ( $existing_transaction->getState() != \VRPayment\Sdk\Model\TransactionState::PENDING ) {
					return;
				}
			...
			$order->add_meta_data( '_vrpayment_gateway_id', $this->gateway->id, true );
			$order->delete_meta_data( '_wc_vrpayment_restocked' );
		} catch ( Exception $e ) {
			$order->update_status( 'failed', $e->getMessage(), 'woo-vrpayment' );
			WooCommerce_VRPayment::instance()->log( $e->getMessage() . "\n" . $e->getTraceAsString() );
			return;
		}
	}
		...
		$payment_meta[ $this->gateway->id ] = array(
			'post_meta' => array(
				'_vrpayment_subscription_space_id' => array(
					'value' => get_post_meta( $subscription->get_id(), '_vrpayment_subscription_space_id', true ),
					'label' => 'VR Payment Space Id',
				),
				'_vrpayment_subscription_token_id' => array(
					'value' => get_post_meta( $subscription->get_id(), '_vrpayment_subscription_token_id', true ),
	...
	public function validate_subscription_payment_meta( $payment_method_id, $payment_meta ) {
		if ( $this->gateway->id === $payment_method_id ) {
			if ( ! isset( $payment_meta['post_meta']['_vrpayment_subscription_space_id']['value'] ) || empty( $payment_meta['post_meta']['_vrpayment_subscription_space_id']['value'] ) ) {
				throw new Exception( esc_html__( 'The VR Payment Space Id value is required.', 'woo-vrpayment' ) );
			} elseif ( get_option( WooCommerce_VRPayment::VRPAYMENT_CK_SPACE_ID ) != $payment_meta['post_meta']['_vrpayment_subscription_space_id']['value'] ) {
				throw new Exception( esc_html__( 'The VR Payment Space Id needs to be in the same space as configured in the main configuration.', 'woo-vrpayment' ) );
			}
			if ( ! isset( $payment_meta['post_meta']['_vrpayment_subscription_token_id']['value'] ) || empty( $payment_meta['post_meta']['_vrpayment_subscription_token_id']['value'] ) ) {
				throw new Exception( esc_html__( 'The VR Payment Token Id value is required.', 'woo-vrpayment' ) );
			}
		}
	}
	...
	public function update_payment_method( $update, $new_payment_method, $subscription ) {
		if ( $this->gateway->id == $new_payment_method ) {
			$update = false;
			add_filter(
				'wc_vrpayment_gateway_result_send_json',
				array(
					$this,
	...
	public function process_subscription_failing_payment_method_updated( $subscription, $renewal_order ) {
		update_post_meta( $subscription->get_id(), '_vrpayment_subscription_space_id', $renewal_order->get_meta( '_vrpayment_subscription_space_id', true ) );
		update_post_meta( $subscription->get_id(), '_vrpayment_subscription_token_id', $renewal_order->get_meta( '_vrpayment_subscription_token_id', true ) );
	}
	...
	private function get_token_data_from_order($order)
	{
		...
		$token_data = [];
		$token_data['_vrpayment_subscription_space_id'] = get_post_meta( $order_id, '_vrpayment_subscription_space_id', true );
		$token_data['_vrpayment_subscription_token_id'] = get_post_meta( $order_id, '_vrpayment_subscription_token_id', true );

		if( ! isset($token_data['_vrpayment_subscription_space_id']) || isset($token_data['_vrpayment_subscription_token_id']) ) {
			...
			$token_data = [];
			$subscription_keys = [
				'_vrpayment_subscription_space_id',
				'_vrpayment_subscription_token_id'
			];
		...
		if( ! isset($token_data['_vrpayment_subscription_space_id']) ) {
			$order->update_status( 'failed', esc_html__( 'No Space Id is found.', 'woo-vrpayment' ) );
			throw new Exception('Missing space id details');
		}

		if( ! isset($token_data['_vrpayment_subscription_token_id']) ) {
			$order->update_status( 'failed', esc_html__( 'No Token Id is found.', 'woo-vrpayment' ) );
			throw new Exception('Missing token id');
		}

		if ( get_option( WooCommerce_VRPayment::VRPAYMENT_CK_SPACE_ID ) != $token_data['_vrpayment_subscription_space_id'] ) {
			$order->update_status( 'failed', esc_html__( 'The token space and the configured space are not equal.', 'woo-vrpayment' ) );
	...
	public function store_gateway_result_in_globals( $result, $subscription ) {
		if ( isset( $result['vrpayment'] ) ) {
			$GLOBALS['_wc_vrpayment_subscription_gateway_result'] = $result;
			return array(
				'result' => $result['result'],
				'redirect' => 'wc_vrpayment_subscription_redirect',
			);
		}
		return $result;
	}
	...
	public function create_json_response( $location, $status ) {
		...
		if ( 'wc_vrpayment_subscription_redirect' == $location && isset( $GLOBALS['_wc_vrpayment_subscription_gateway_result'] ) ) {
			wp_send_json( $GLOBALS['_wc_vrpayment_subscription_gateway_result'] );
			exit;
		}
		return $location;
	}
