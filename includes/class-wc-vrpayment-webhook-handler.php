...
/**
 * Class WC_VRPayment_Unique_Id.
 * This class handles the webhooks of VRPayment
 *
 * @class WC_VRPayment_Unique_Id
 */
class WC_VRPayment_Webhook_Handler {
	...
	public static function init() {
		add_action(
			'woocommerce_api_vrpayment_webhook',
			array(
				__CLASS__,
				'process',
			)
	...
	public static function process() {
		...
		try {
			...
			$request = new WC_VRPayment_Webhook_Request( json_decode( $clean_data ) );
			$client = WC_VRPayment_Helper::instance()->get_api_client();
			$webhook_service = WC_VRPayment_Service_Webhook::instance();
			...
			if ( ! empty( $signature ) && $client->getWebhookEncryptionService()->isContentValid( $signature, $clean_data ) ) {
				WC_VRPayment_Webhook_Strategy_Manager::instance()->process( $request );
			}
...
WC_VRPayment_Webhook_Handler::init();
