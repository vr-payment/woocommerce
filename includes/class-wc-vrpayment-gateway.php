...
class WC_VRPayment_Gateway extends WC_Payment_Gateway {
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
				'title' => esc_html__( 'Enable/Disable', 'woo-vrpayment' ),
				'type' => 'checkbox',
			...
			'title_value' => array(
				'title' => esc_html__( 'Title', 'woo-vrpayment' ),
				'type' => 'info',
			...
			'description_value' => array(
				'title' => esc_html__( 'Description', 'woo-vrpayment' ),
				'type' => 'info',
	...
	public function is_available() {
		...
		try {
			...
			$gateway_available = WC()->session && WC()->session->has_session()
				? WC()->session->get( 'vrpayment_payment_gateways' )
				: array();

			if ( isset( $gateway_available[ $this->vrp_payment_method_configuration_id ] ) ) {
			...
			if ( apply_filters(
				...
				is_admin() || ! is_checkout() || ( isset( $GLOBALS['_vrpayment_calculating'] ) && $GLOBALS['_vrpayment_calculating'] ),
				$this
			) ) {
			...
			if ( is_checkout() && isset( $wp->query_vars['order-received'] ) ) {
				return ! empty( $gateway_available[ $this->vrp_payment_method_configuration_id ] );
			}

			// Step 6: Handle "order pay" endpoint
			if ( apply_filters( 'wc_vrpayment_is_order_pay_endpoint', is_checkout_pay_page() ) ) {
	...
	protected function get_safe_possible_payment_methods_for_cart() {
		try {
			return WC_VRPayment_Service_Transaction::instance()->get_possible_payment_methods_for_cart();
	...
	protected function get_safe_possible_payment_methods_for_order( $order ) {
		try {
			return WC_VRPayment_Service_Transaction::instance()->get_possible_payment_methods_for_order( $order );
		} catch ( WC_VRPayment_Exception_Invalid_Transaction_Amount $e ) {
	...
	public function enqueue_checkout_scripts() {
		...
		if ( ! wp_script_is( 'vrpayment-remote-checkout-js', 'enqueued' ) ) {
            ...
            if (get_option( WooCommerce_VRPayment::VRPAYMENT_CK_INTEGRATION ) == WC_VRPayment_Integration::VRPAYMENT_PAYMENTPAGE) {
                return false;
			}
