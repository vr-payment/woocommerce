...
if ( ! class_exists( 'WooCommerce_VRPayment' ) ) {

	/**
	 * Main WooCommerce VRPayment Class
	 *
	 * @class WooCommerce_VRPayment
	 */
	final class WooCommerce_VRPayment {

		const VRPAYMENT_CK_SPACE_ID = 'wc_vrpayment_space_id';
		const VRPAYMENT_CK_SPACE_VIEW_ID = 'wc_vrpayment_space_view_id';
		const VRPAYMENT_CK_APP_USER_ID = 'wc_vrpayment_application_user_id';
		const VRPAYMENT_CK_APP_USER_KEY = 'wc_vrpayment_application_user_key';
		const VRPAYMENT_CK_CUSTOMER_INVOICE = 'wc_vrpayment_customer_invoice';
		const VRPAYMENT_CK_CUSTOMER_PACKING = 'wc_vrpayment_customer_packing';
		const VRPAYMENT_CK_SHOP_EMAIL = 'wc_vrpayment_shop_email';
		 ...
		 *
		 * @return WooCommerce_VRPayment - Main instance.
		 */
		...
		/**
		 * WooCommerce VRPayment Constructor.
		 */
		...
		/**
		 * Define WC VRPayment Constants.
		 */
		protected function define_constants() {
			$this->define( 'WC_VRPAYMENT_PLUGIN_FILE', __FILE__ );
			...
			$this->define( 'WC_VRPAYMENT_REQUIRED_WC_SUBSCRIPTION_VERSION', self::REQUIRED_WC_SUBSCRIPTION_VERSION );
		}
		...
		protected function includes() {
			 ...
			 * Class autoloader.
			 */
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-autoloader.php';
			require_once WC_VRPAYMENT_ABSPATH . 'vrpayment-sdk/autoload.php';

			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-migration.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-email.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-return-handler.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-webhook-handler.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-unique-id.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-customer-document.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-cron.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-order-status-adapter.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/packages/coupon/class-wc-vrpayment-packages-coupon-discount.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/packages/gift-card/class-wc-vrpayment-packages-gift-card.php';
			if ( defined( 'WP_CLI' ) && WP_CLI ) {
				require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-commands.php';
			}

			if ( is_admin() ) {
				require_once WC_VRPAYMENT_ABSPATH . 'includes/admin/class-wc-vrpayment-admin.php';
			}
		}
		...
		protected function init_hooks() {
			...
			add_action(
				'wp_ajax_vrpayment_is_payment_method_available',
				array(
					'WC_VRPayment_Blocks_Support',
					'is_payment_method_available',
				)
			);
			add_action(
				'wp_ajax_nopriv_vrpayment_is_payment_method_available',
				array(
					'WC_VRPayment_Blocks_Support',
					'is_payment_method_available',
				)
			...
			add_action(
				'wp_ajax_vrpayment_get_payment_methods',
				array(
					'WC_VRPayment_Blocks_Support',
			...
			add_action(
				'wp_ajax_nopriv_vrpayment_get_payment_methods',
				array(
					'WC_VRPayment_Blocks_Support',
					'get_payment_methods_json',
				)
			...
			add_action(
				'wp_ajax_vrpayment_custom_order_status_save_changes',
				array(
					$this,
			...
			add_action(
				'wp_ajax_vrpayment_custom_order_status_delete',
				array(
					$this,
		...
		public static function plugin_deactivate() {
			...
			$old_plugin_prefix = WC_VRPayment_Migration::VRPAYMENT_DEPRECATED_PLUGIN_PREFIX;
			$plugin_current_version = self::get_installed_plugin_version( $old_plugin_prefix . 'vrpayment' ); // The slug of the old plugin.

			// Check if the plugin version is lower than 3.1.0.
			if ( version_compare( $plugin_current_version, self::VRPAYMENT_UPGRADE_VERSION, '<' ) ) {
		...
		public function secure_redirect_order_confirmed( $order_id ) {
			...
			$wc_service_transaction = WC_VRPayment_Service_Transaction::instance();
			$sdk_service_transaction = new \VRPayment\Sdk\Service\TransactionService( WC_VRPayment_Helper::instance()->get_api_client() );
			$wc_transaction_info = WC_VRPayment_Entity_Transaction_Info::load_by_order_id( $order_id );

			if ( property_exists( $wc_transaction_info, 'get_transaction_id' ) ) {
				$state = $sdk_service_transaction->read( get_option( self::VRPAYMENT_CK_SPACE_ID ), $wc_transaction_info->get_transaction_id() )->getState();
				if ( \VRPayment\Sdk\Model\TransactionState::CONFIRMED === $state ) {
					wp_redirect($wc_service_transaction->get_payment_page_url(get_option(self::VRPAYMENT_CK_SPACE_ID), $wc_transaction_info->get_transaction_id())); //phpcs:ignore
					exit;
				}
			}
		}
		...
		public static function migrate_plugin_data_on_activation() {
			$old_option_prefix = WC_VRPayment_Migration::VRPAYMENT_DEPRECATED_TABLE_PREFIX;
			$old_plugin_prefix = WC_VRPayment_Migration::VRPAYMENT_DEPRECATED_PLUGIN_PREFIX;

			$plugin_slug = $old_plugin_prefix . 'vrpayment'; // The slug of the old plugin.
			 ...
			 */
			if ( version_compare( $installed_version, self::VRPAYMENT_UPGRADE_VERSION, '>=' ) ) {
				return;
			}

			global $wpdb;
			$options_to_migrate = [
				$old_option_prefix . WC_VRPayment_Migration::VRPAYMENT_CK_DB_VERSION,
				$old_option_prefix . WC_VRPayment_Service_Manual_Task::VRPAYMENT_CONFIG_KEY,
				$old_option_prefix . self::VRPAYMENT_CK_SPACE_ID,
				$old_option_prefix . self::VRPAYMENT_CK_SPACE_VIEW_ID,
				$old_option_prefix . self::VRPAYMENT_CK_APP_USER_ID,
				$old_option_prefix . self::VRPAYMENT_CK_APP_USER_KEY,
				$old_option_prefix . self::VRPAYMENT_CK_CUSTOMER_INVOICE,
				$old_option_prefix . self::VRPAYMENT_CK_CUSTOMER_PACKING,
				$old_option_prefix . self::VRPAYMENT_CK_SHOP_EMAIL,
			...
			foreach ( $options_to_migrate as $option_name ) {
				...
				if ( false !== $option_value ) {
					// Rename the options to the new prefix 'vrpayment_'.
					$new_option_name = str_replace( $old_option_prefix . 'vrpayment_', 'vrpayment_', $option_name );
		...
		public function loaded() {
			...
			add_action('plugins_loaded', function () {
				if (class_exists('WC_Payment_Gateway')) {
					...
					add_filter(
					  ...
					  array(
						WooCommerce_VRPayment::instance(),
						'add_gateways',
					  )
					);
				}
			...
			add_filter(
			 	...
			 	function($value, $order) {
					...
					if (substr_count($order_data['payment_method'], "vrpayment")) {
			...
			add_filter('the_content', function($content) {
				if ( is_checkout() && ! is_order_received_page() ) {
					...
					$payment_methods = WC_VRPayment_Blocks_Support::get_payment_methods();
					$json_data = json_encode( $payment_methods );
					$content .= '<div id="vrpayment-payment-methods" data-json="' . esc_attr( $json_data ) . '"></div>';
				}
		...
		public function maybe_boot_subscriptions() {
			...
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-subscription.php';
			require_once WC_VRPAYMENT_ABSPATH . 'includes/class-wc-vrpayment-subscription-gateway.php';
			if ( class_exists( 'WC_VRPayment_Subscription' ) ) {
				WC_VRPayment_Subscription::instance()->boot();
			}
		}
		...
		public static function is_subscriptions_compatible(): bool  {
			...
			if ( version_compare( $woocommerce_subscriptions_data['Version'], WC_VRPAYMENT_REQUIRED_WC_SUBSCRIPTION_VERSION, '<' ) ) {
				return false;
			}
			return true;
		}
		...
		public function after_remove_product_from_cart( $removed_cart_item_key, $cart ) {
			...
			if ( $this->is_product_type_subscription( $product ) ) {
				...
				$service_transaction = WC_VRPayment_Service_Transaction::instance();
	...
	public function add_order_statuses( $order_statuses ) {
		$order_statuses['wc-vrpaym-redirected'] = _x( 'Redirected', 'Order status', 'woo-vrpayment' );
		$order_statuses['wc-vrpaym-waiting'] = _x( 'Waiting', 'Order status', 'woo-vrpayment' );
		$order_statuses['wc-vrpaym-manual'] = _x( 'Manual Decision', 'Order status', 'woo-vrpayment' );
	...
	public function valid_order_statuses_for_payment( $statuses, $order = null ) { //phpcs:ignore
		...
		public function custom_order_status_save_changes() {
			...
			if ( ! isset( $_POST['vrpayment_order_statuses_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['vrpayment_order_statuses_nonce'] ) ), 'vrpayment_order_statuses_nonce' ) ) {  //phpcs:ignore
			...
			if ( isset( $data['key'] ) ) {
				...
				$sanitized_key_underscore = str_replace( [' ', '-'], '_', WC_VRPayment_Order_Status_Adapter::VRPAYMENT_CUSTOM_ORDER_STATUS_PREFIX . $first_key );
		...
		public function custom_order_status_delete() {
			...
			// Verify nonce.
			if ( ! isset( $_POST['vrpayment_order_statuses_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['vrpayment_order_statuses_nonce'] ) ), 'vrpayment_order_statuses_nonce' ) ) {
				wp_send_json_error( 'Invalid request: missing nonce value', 403 );
			}
			...
			// Check if this order status is being used in wp_options with key prefix "vrpayment_order_status_mapping_".
			$like_pattern = WC_VRPayment_Order_Status_Adapter::VRPAYMENT_ORDER_STATUS_MAPPING_PREFIX . '%';
			...
			if ( $is_used > 0 ) {
				...
				wp_send_json_error( ['message' => __( 'Cannot delete: this status is currently in use.', 'woo-vrpayment' ), 'key' => ucfirst( $sanitized_key )], 400 );
			}

			 // Get the custom order status in wp_options with key prefix "vrpayment_order_status_mapping_".
			$like_pattern = WC_VRPayment_Order_Status_Adapter::VRPAYMENT_CUSTOM_ORDER_STATUS_PREFIX . '%';
			...
			wp_send_json_success( ['message' => __( 'Deleted successfully', 'woo-vrpayment' ), 'key' => ucfirst( $sanitized_key )] );
		}
		...
		public function set_device_id_cookie() {
			if(!headers_sent()) {
				$value = WC_VRPayment_Unique_Id::get_uuid();
				if ( isset( $_COOKIE['wc_vrpayment_device_id'] ) && ! empty( $_COOKIE['wc_vrpayment_device_id'] ) ) {
					$value = sanitize_text_field( wp_unslash( $_COOKIE['wc_vrpayment_device_id'] ) );
				}
				setcookie( 'wc_vrpayment_device_id', $value, time() + YEAR_IN_SECONDS, '/' );
			}
		}
		...
		public function set_js_async( $tag, $handle, $src ) { //phpcs:ignore
			$async_script_handles = array( 'vrpayment-device-id-js' );
		...
		public function enqueue_javascript_script() {
			if ( is_cart() || is_checkout() ) {
				$unique_id = isset( $_COOKIE['wc_vrpayment_device_id'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['wc_vrpayment_device_id'] ) ) : null;
				$space_id = get_option( self::VRPAYMENT_CK_SPACE_ID );
				$script_url = WC_VRPayment_Helper::instance()->get_base_gateway_url() . 's/' .
						$space_id . '/payment/device.js?sessionIdentifier=' .
						$unique_id;
				wp_enqueue_script( 'vrpayment-device-id-js', $script_url, array(), $this->get_version(), true );
			}
		}
		...
		public function enqueue_stylesheets() {
			if ( is_checkout() ) {
				wp_enqueue_style( 'vrpayment-checkout-css', $this->plugin_url() . '/assets/css/checkout.css', array(), $this->get_version() );
			}
		}
		...
		public function before_calculate_totals( $cart ) { //phpcs:ignore
			$GLOBALS['vrpayment_calculating'] = true;
		}
		...
		public function after_calculate_totals( $cart ) { //phpcs:ignore
			unset( $GLOBALS['vrpayment_calculating'] );
		}
		...
		public function add_gateways( $methods ) {
			...
			require_once __DIR__ . '/includes/class-wc-vrpayment-zero-gateway.php';
			$methods[] = 'WC_VRPayment_Zero_Gateway';

			$method_configurations = WC_VRPayment_Entity_Method_Configuration::load_by_states_and_space_id(
				$space_id,
				array(
					WC_VRPayment_Entity_Method_Configuration::VRPAYMENT_STATE_ACTIVE,
					WC_VRPayment_Entity_Method_Configuration::VRPAYMENT_STATE_INACTIVE,
				)
			);
			try {
				foreach ( $method_configurations as $configuration ) {
					$gateway = new WC_VRPayment_Gateway( $configuration );
					$methods[] = apply_filters('wc_vrpayment_enhance_gateway', $gateway);
				}
		...
		public function update_additional_customer_data( $arguments ) {
			...
			$custom_billing_date_of_birth_field_name = apply_filters( 'wc_vrpayment_billing_date_of_birth_field_name', '' );
			...
			$billing_gender = '';
			$custom_billing_gender_field_name = apply_filters( 'wc_vrpayment_billing_gender_field_name', '' );
			...
			if ( ! empty( $billing_date_of_birth ) ) {
				WC()->customer->add_meta_data( '_vrpayment_billing_date_of_birth', $billing_date_of_birth, true );
			}
			if ( ! empty( $billing_gender ) ) {
				WC()->customer->add_meta_data( '_vrpayment_billing_gender', $billing_gender, true );
			}

			if ( ! empty( $post_data['ship_to_different_address'] ) && ! wc_ship_to_billing_address_only() ) {
				$shipping_date_of_birth = '';
				$custom_shipping_date_of_birth_field_name = apply_filters( 'wc_vrpayment_shipping_date_of_birth_field_name', '' );
				...
				$shipping_gender = '';
				$custom_shipping_gender_field_name = apply_filters( 'wc_vrpayment_shipping_gender_field_name', '' );
				...
				if ( ! empty( $shipping_date_of_birth ) ) {
					WC()->customer->add_meta_data( '_vrpayment_shipping_date_of_birth', $shipping_date_of_birth, true );
				}
				if ( ! empty( $shipping_gender ) ) {
					WC()->customer->add_meta_data( '_vrpayment_shipping_gender', $shipping_gender, true );
				}
			} else {
				if ( ! empty( $billing_date_of_birth ) ) {
					WC()->customer->add_meta_data( '_vrpayment_shipping_date_of_birth', $billing_date_of_birth, true );
				}
				if ( ! empty( $billing_gender ) ) {
					WC()->customer->add_meta_data( '_vrpayment_shipping_gender', $billing_gender, true );
				}
			}
		}
		...
		public function log( $message, $level = WC_Log_Levels::WARNING ) {
			...
			$this->logger->log(
				...
				$message,
				array(
					'source' => 'woo-vrpayment',
				)
			);

			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'Woocommerce VRPayment: ' . $message ); //phpcs:ignore
			}
		}
		...
		protected function update_attribute_options( $attribute_id, $send ) {
			$attribute_options = WC_VRPayment_Entity_Attribute_Options::load_by_attribute_id( $attribute_id );
		...
		public function woocommerce_rest_insert_product_attribute( $attribute, $request, $create ) { //phpcs:ignore
			if ( isset( $request['vrpayment_attribute_option_send'] ) ) {
				if ( $request['vrpayment_attribute_option_send'] ) {
		...
		public function woocommerce_rest_prepare_product_attribute( $response, $item, $request ) {
			...
			if ( 'view' === $context || 'edit' === $context ) {
				...
				$attribute_options = WC_VRPayment_Entity_Attribute_Options::load_by_attribute_id( $item->attribute_id );
				$data['vrpayment_attribute_option_send'] = $attribute_options->get_id() > 0 && $attribute_options->get_send();
	...
	function WC_VRPayment_Blocks_Support() { //phpcs:ignore
		if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
			require_once __DIR__ . '/includes/class-wc-vrpayment-blocks-support.php';
